...
class EnhancedMessageRenderer:
    ...
    async def get_enhanced_avatar(self, user: Union[discord.User, discord.Member]) -> Image.Image:
        """
    ...
    async def _download_avatar(self, user: Union[discord.User, discord.Member]) -> Image.Image:
        """
        下載用戶頭像
    ...
    def _create_enhanced_avatar(self, avatar: Image.Image, user: Union[discord.User, discord.Member]) -> Image.Image:
        """
    ...
    async def download_attachment(self, attachment: discord.Attachment) -> Optional[Image.Image]:
        ...
        Returns:
            Optional[Image.Image]: 附件圖片，如果下載失敗則為 None
        """
        try:
            # 檢查是否為圖片
    ...
    def _sanitize_external_emoji(self, text: str, guild: Optional[discord.Guild]) -> str:
        """
        處理外部表情符號
    ...
    async def render_messages(self, messages: List[discord.Message]) -> Optional[str]:
        ...
        Returns:
            Optional[str]: 圖片檔案路徑，如果渲染失敗則為 None
        """
        if not messages:
            return None
