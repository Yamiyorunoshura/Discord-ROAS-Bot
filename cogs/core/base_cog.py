...
class BaseCog(commands.Cog):
    ...
    async def resolve_service(self, service_type: Type[T], scope: str | None = None) -> T:
        """
        解析服務實例
    ...
    def register_service(self, service_type: Type[T], implementation_type: Type[T | None] = None, 
...
async def inject_service(service_type: Type[T], scope: str | None = None) -> T:
    """
    全局服務注入函數
...
def requires_service(service_type: Type[T], scope: str | None = None):
    """
    服務依賴裝飾器
...
class StandardPanelView(BasePanelView):
    ...
    def add_page(self, page_id: str, title: str, description: str, 
                 embed_builder: Callable, components: List | None = None):
        """
        添加頁面
