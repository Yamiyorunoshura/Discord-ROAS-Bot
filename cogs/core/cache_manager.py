...
from typing import (
    ...
    Callable, TypeVar, Generic, AsyncGenerator
)
    ...
    async def analyze_patterns(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "avg_accesses_per_key": sum(key_frequency.values()) / len(key_frequency) if key_frequency else 0
            }
...
class SmartCacheStrategy:
    ...
    async def should_evict(self, entries: Dict[str, CacheEntry], max_size: int) -> List[str]:
        """智能選擇需要淘汰的條目"""
...
class MemoryCacheBackend(CacheBackend):
    ...
    async def _evict_entries(self):
        ...
        if self.strategy == CacheStrategy.ADAPTIVE:
            # 使用智能策略
            keys_to_evict = await self.smart_strategy.should_evict(self._cache, self.max_size - 1)
            ...
            if self.strategy == CacheStrategy.LRU:
                ...
                # 移除最不常用的
                min_freq_key = min(self._cache.keys(), 
    ...
    async def get_detailed_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            base_stats = {
                ...
                "avg_entry_size": self.stats.total_size / self.stats.entry_count if self.stats.entry_count > 0 else 0
            }
        ...
        async with self._lock:
            ...
            try:
                ...
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    entry.key, value_blob, entry.timestamp, entry.access_count,
                    entry.last_access, entry.ttl, entry.size, metadata_json,
                    entry.hit_count, entry.load_time
                ))
        ...
        return {
            ...
            "combined": {
                ...
                "combined_hit_rate": (l1_stats["hits"] + l2_stats["hits"]) / 
                                   ...
                                   if (l1_stats["hits"] + l1_stats["misses"] + l2_stats["hits"] + l2_stats["misses"]) > 0 else 0,
...
# 全域緩存管理器
_global_cache_manager: Optional[MultiLevelCache] = None
