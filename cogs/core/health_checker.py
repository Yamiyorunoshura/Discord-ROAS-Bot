...
class HealthChecker:
    ...
    def update_module_status(self, name: str, status: HealthStatus, 
    ...
    async def _check_log_analysis(self) -> HealthCheckResult:
        ...
        try:
            ...
            if health_report.get("overall_status") == "warning":
                status = HealthStatus.WARNING
                message = f"日誌中發現問題: {len(health_report.get('issues', []))} 個問題"
    ...
    def _generate_recommendations(self, results: List[HealthCheckResult]) -> List[str]:
            ...
            if result.status == HealthStatus.CRITICAL:
                if result.name == "system_resources":
                    if "CPU" in result.message:
                        recommendations.append("檢查 CPU 使用率過高的進程，考慮優化或重啟")
        ...
        if not recommendations:
            if any(r.status in [HealthStatus.CRITICAL, HealthStatus.WARNING] for r in results):
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        latest_report = self.generate_health_report(recent_results[-10:])  # 最近10次檢查
