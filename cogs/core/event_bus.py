...
from typing import (
    ...
    Callable, Awaitable, Set, AsyncGenerator, Deque
)
...
class Event:
    ...
    def __post_init__(self):
        ...
        # 自動生成批處理鍵
        if self.batch_key is None and self.processing_mode == EventProcessingMode.BATCHED:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Event':
        ...
        return cls(
            ...
            processing_mode=EventProcessingMode(data.get('processing_mode', EventProcessingMode.IMMEDIATE.value)),
...
class EventMetrics:
    ...
    async def record_event_processed(self, event: Event, processing_time: float, success: bool = True):
        """記錄事件處理"""
        async with self._lock:
            if success:
    ...
    async def record_batch_processed(self, batch: EventBatch, processing_time: float):
        ...
        async with self._lock:
            ...
            for event in batch.events:
                await self.record_event_processed(event, processing_time / len(batch.events))
        ...
        async with self._lock:
            ...
            return {
                ...
                "success_rate": (
                    ...
                    if (self.total_events_processed + self.total_events_failed) > 0 else 1.0
                ),
                "avg_processing_time_ms": avg_processing_time * 1000,
                "events_per_second": self.total_events_published / uptime if uptime > 0 else 0.0,
                "events_by_type": dict(self.events_by_type),
                "events_by_priority": dict(self.events_by_priority),
                "batch_count": self.batch_count,
                "avg_batch_size": avg_batch_size,
                "error_rate": self.error_count / self.total_events_published if self.total_events_published > 0 else 0.0
            }
...
class EventRouter:
    ...
    async def route_event(self, event: Event, subscriptions: Dict[str, EventSubscription]) -> List[str]:
        """路由事件到合適的訂閱者"""
        async with self._lock:
            matched_subscribers = []
        ...
        async with self._lock:
            ...
            if subscriber_id in self.performance_cache:
                self.performance_cache[subscriber_id] = (
                    0.7 * self.performance_cache[subscriber_id] + 
                    0.3 * processing_time
                )
            else:
...
class EventCompressor:
    ...
    def register_compression_rule(self, event_type: str, rule: Callable[[List[Event]], List[Event]]):
        """註冊壓縮規則"""
...
class EventPersistence(ABC):
    ...
    @abstractmethod
    async def load_events(self, event_type: str | None = None, 
    ...
    async def delete_events(self, event_ids: List[str]) -> int:
        ...
        async with self._lock:
            ...
            self.events = [
                ...
                if event.event_id not in event_ids or (deleted_count := deleted_count + 1, False)[1]
            ]
...
class EventBus:
    ...
    def _setup_default_compression_rules(self):
        ...
        self.compressor.register_compression_rule("counter_increment", merge_counter_events)
    ...
    def subscribe(self, event_types: List[str], 
                 handler: EventHandler,
                 filters: List[Callable[['Event | None'], bool]] = None,
                 priority: EventPriority = EventPriority.NORMAL,
                 max_retries: int = 3,
                 subscriber_id: str | None = None,
                 processing_mode: EventProcessingMode = EventProcessingMode.IMMEDIATE,
    ...
    async def publish(self, event: Event, persist: bool = True) -> bool:
        ...
        try:
            ...
            # 根據處理模式路由事件
            if event.processing_mode == EventProcessingMode.BATCHED and self.enable_batch_processing:
    ...
    async def _process_batch(self, batch: EventBatch):
        ...
        try:
            ...
            logger.debug(f"【事件總線】批次處理完成: {batch.batch_key} ({len(batch.events)} 事件)")
            
        except Exception as e:
            logger.error(f"【事件總線】批次處理失敗: {e}")
    
    async def _execute_batch_handler(self, subscription: EventSubscription, events: List[Event]):
        """執行批次處理器"""
        try:
            # 如果處理器支持批次處理，傳遞事件列表
            ...
            logger.error(f"【事件總線】批次處理器執行失敗 {subscription.subscriber_id}: {e}")
        ...
        while retry_count <= subscription.max_retries:
            try:
                ...
                if subscription.subscriber_id:
                    await self.router.update_performance(subscription.subscriber_id, processing_time)
                await self.metrics.record_event_processed(event, processing_time, True)
    ...
    async def get_event_history(self, event_type: str | None = None,
                              limit: int = 100) -> List[Event]:
        """獲取事件歷史"""
        return await self.persistence.load_events(
...
# 便捷函數
async def publish_event(event_type: str, data: Dict[str, Any | None] = None, 
                       source: str | None = None, target: str | None = None,
                       priority: EventPriority = EventPriority.NORMAL,
                       processing_mode: EventProcessingMode = EventProcessingMode.IMMEDIATE) -> bool:
    """發布事件的便捷函數"""
    event = Event(
        event_type=event_type,
        data=data or {},
