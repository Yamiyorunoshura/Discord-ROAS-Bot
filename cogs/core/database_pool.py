...
class PoolMetrics:
    ...
    async def record_connection_created(self):
        ...
        async with self._lock:
            ...
            self.peak_active_connections = max(self.peak_active_connections, self.active_connections)
    ...
    async def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            avg_query_time = (self.total_query_time / self.total_queries) if self.total_queries > 0 else 0.0
            
            # 計算最近查詢性能
            recent_queries = [q for q in self.query_history if time.time() - q['timestamp'] < 300]  # 最近5分鐘
            recent_avg_time = statistics.mean([q['duration'] for q in recent_queries]) if recent_queries else 0.0
            recent_success_rate = sum(1 for q in recent_queries if q['success']) / len(recent_queries) if recent_queries else 1.0
            ...
            return {
                ...
                "error_rate": self.connection_errors / self.total_connections_created if self.total_connections_created > 0 else 0.0
            }
...
class LoadBalancer:
    ...
    async def select_connection(self, connections: List['PooledConnection']) -> Optional['PooledConnection']:
        """選擇最佳連接"""
        if not connections:
            return None
        
        available_connections = [conn for conn in connections if conn.is_healthy and not conn.in_use]
    ...
    def _round_robin_select(self, connections: List['PooledConnection']) -> 'PooledConnection':
        """輪詢選擇"""
        self.last_used_index = (self.last_used_index + 1) % len(connections)
        return connections[self.last_used_index]
    
    def _least_used_select(self, connections: List['PooledConnection']) -> 'PooledConnection':
        """選擇使用次數最少的連接"""
        return min(connections, key=lambda conn: conn.metrics.total_queries)
    
    def _random_select(self, connections: List['PooledConnection']) -> 'PooledConnection':
        """隨機選擇"""
        import random
        return random.choice(connections)
    
    async def _adaptive_select(self, connections: List['PooledConnection']) -> 'PooledConnection':
        """自適應選擇 - 基於性能指標"""
        # 計算每個連接的綜合分數
...
class ConnectionPrewarmer:
    ...
    async def _prewarm_connections(self, db_path: str):
        ...
        try:
            for i in range(self.config.warmup_connections):
    ...
    async def _perform_recovery(self, db_path: str, error: Exception) -> bool:
        ...
        try:
            ...
            if db_path in pool._connections:
                failed_connections = [
                    conn for conn in pool._connections[db_path] 
                    if not conn.is_healthy or conn.metrics.status == ConnectionStatus.FAILED
                ]
...
class PooledConnection:
    ...
    def __init__(self, connection: aiosqlite.Connection, db_path: str, pool_ref: weakref.ref):
...
class DatabaseConnectionPool:
    ...
    async def get_connection(self, db_path: str) -> PooledConnection:
        ...
        async with self._locks[db_path]:
            ...
            # 如果仍然沒有可用連接，等待或拋出異常
            raise RuntimeError(f"無法獲取資料庫連接: {db_path} (活躍連接: {len(connections)}/{self.config.max_connections})")
    ...
    async def _create_connection(self, db_path: str) -> PooledConnection:
        ...
        while retry_count < self.config.retry_attempts:
            try:
                ...
                logger.debug(f"【連接池】新連接已創建: {pooled_conn.connection_id} for {db_path}")
                ...
                logger.warning(f"【連接池】創建連接失敗 (嘗試 {retry_count}/{self.config.retry_attempts}): {e}")
                
                if retry_count < self.config.retry_attempts:
                    await asyncio.sleep(self.config.retry_delay * retry_count)
                else:
                    # 嘗試自動恢復
                    if await self.auto_recovery.attempt_recovery(db_path, e):
                        logger.info(f"【連接池】自動恢復成功，重試創建連接")
    ...
    async def execute(self, db_path: str, query: str, params: tuple = ()) -> aiosqlite.Cursor:
        """執行查詢的便捷方法"""
