...
class ExampleCog(BaseCog):
    ...
    async def service_info_command(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(
                name="Cog名稱",
                value=service_info.get("cog_name", "未知"),
                inline=True
            )
            
            embed.add_field(
                name="初始化狀態",
                value="✅ 已初始化" if service_info.get("initialized") else "❌ 未初始化",
                inline=True
            )
            ...
            if container_info:
                ...
                if services:
                    service_names = [s.get("service_type", "未知") for s in services[:5]]  # 只顯示前5個
                    embed.add_field(
                        name="註冊的服務",
                        value="\n".join(service_names) + ("..." if len(services) > 5 else ""),
                        inline=False
                    )
    ...
    async def scope_test_command(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(
                name="作用域1 - 服務A",
                value=f"ID: {service1_a.id}",
                inline=True
            )
            
            embed.add_field(
                name="作用域1 - 服務B",
                value=f"ID: {service1_b.id}",
                inline=True
            )
            ...
            embed.add_field(
                name="作用域2 - 服務A",
                value=f"ID: {service2_a.id}",
                inline=True
            )
            
            embed.add_field(
                name="作用域2 - 服務B",
                value=f"ID: {service2_b.id}",
                inline=True
            )
