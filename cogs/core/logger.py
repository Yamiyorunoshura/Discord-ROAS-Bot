...
class PerformanceMonitor:
    ...
    def record_metrics(self, response_time_ms: float | None = None,
                      active_connections: int | None = None,
                      error_rate: float | None = None):
        """記錄性能指標"""
        try:
    ...
    def generate_health_report(self) -> Dict[str, Any]:
        ...
        for log_file in self.logs_dir.glob("*.log"):
            ...
            if analysis.get("error_count", 0) > 10:  # 1小時內超過10個錯誤
                ...
                report["issues"].append(f"{log_file.name}: 錯誤率過高 ({analysis['error_count']} 錯誤/小時)")
...
class DiscordBotLogger:
    ...
    def _create_file_handler(self, filename: str, max_bytes: int | None = None, 
    ...
    def get_logger(self, name: str, filename: str | None = None, 
                   level: int | None = None, max_bytes: int | None = None, 
                   backup_count: int | None = None,
    ...
    def log_startup(self, module_name: str, message: str, duration_ms: float | None = None):
        """
        記錄模塊啟動信息
