...
from .error_handler import create_error_handler, ErrorCodes
...
class StartupManager:
    ...
    def register_module(self, name: str, path: str, priority: int = 0, 
    ...
    def _resolve_load_order(self) -> List[str]:
            ...
            if module:
                for dep in module.dependencies:
                    if dep in self.modules:
                        ...
                        self.logger.warning(f"模組 {module_name} 依賴的模組 {dep} 不存在")
        ...
        sorted_modules = sorted(
            self.modules.keys(), 
            key=lambda x: (self.modules[x].priority, x)
        )
    ...
    async def _load_module_with_retry(self, module_name: str) -> bool:
        ...
        for attempt in range(module.max_retries + 1):
            ...
            module.status = ModuleStatus.RETRYING if attempt > 0 else ModuleStatus.LOADING
            ...
            try:
                ...
                # 載入模組
                self.logger.info(f"🔄 正在載入模組：{module_name} (嘗試 {attempt + 1}/{module.max_retries + 1})")
                ...
                self.logger.info(f"✅ 模組載入成功：{module_name} ({module.load_time:.2f}s)")
                return True
                ...
                if attempt < module.max_retries:
                    ...
                    self.logger.warning(f"⚠️ 模組 {module_name} 載入失敗，{backoff_time:.1f}s 後重試：{exc}")
    ...
    async def _load_modules_batch(self, module_names: List[str]) -> List[bool]:
        ...
        for module_name, task in tasks:
            try:
                ...
                self._notify_progress(current_loaded, self.stats.total_modules, module_name)
    ...
    async def start_all_modules(self) -> StartupStats:
        ...
        try:
            ...
            for priority in sorted(priority_groups.keys()):
                ...
                # 先載入關鍵模組（序列載入）
                if critical_modules:
                    self.logger.info(f"🔑 載入關鍵模組 (優先級 {priority})：{', '.join(critical_modules)}")
                ...
                # 並行載入普通模組
                if normal_modules:
                    self.logger.info(f"⚡ 並行載入模組 (優先級 {priority})：{', '.join(normal_modules)}")
    ...
    def _log_final_stats(self):
        ...
        # 詳細模組資訊
        loaded_modules = [m for m in self.modules.values() if m.status == ModuleStatus.LOADED]
        failed_modules = [m for m in self.modules.values() if m.status in [ModuleStatus.FAILED, ModuleStatus.CRITICAL_FAILED]]
        
        if loaded_modules:
            self.logger.info("\n✅ 成功載入的模組：")
            for module in sorted(loaded_modules, key=lambda x: x.priority):
                critical_mark = "🔑" if module.is_critical else "📦"
                self.logger.info(f"  {critical_mark} {module.name} ({module.load_time:.2f}s) - {module.description}")
        ...
        if failed_modules:
            ...
            for module in failed_modules:
                ...
                self.logger.info(f"  {critical_mark} {module.name}{retry_info}: {module.error}")
    ...
    def get_module_status(self, module_name: str) -> Optional[ModuleStatus]:
        ...
        Returns:
            Optional[ModuleStatus]: 模組狀態，如果模組不存在則返回 None
        """
        module = self.modules.get(module_name)
    ...
    def get_loaded_modules(self) -> List[str]:
        ...
        Returns:
            List[str]: 已載入的模組名稱列表
        """
        return [name for name, module in self.modules.items() 
                if module.status == ModuleStatus.LOADED]
...
# 工具函數
def print_progress_bar(current: int, total: int, module_name: str = "", width: int = 20):
    """
    列印進度條
