...
class WelcomeCache:
    ...
    def __init__(self, timeout: int = 3600, max_size: int = 50):
        ...
        self._cache: Dict[int, Tuple[float, io.BytesIO]] = {}  # guild_id -> (timestamp, image)
    
    def get(self, guild_id: int) -> Optional[io.BytesIO]:
        """
        從快取中取得圖片
        ...
        Returns:
            Optional[io.BytesIO]: 快取的圖片，如果不存在或已過期則為 None
        """
    ...
    def clear(self, guild_id: Optional[int] = None) -> None:
        """
        清除快取
    ...
    def cleanup(self) -> None:
        ...
        current_time = time.time()
        expired_guilds = [
            guild_id for guild_id, (timestamp, _) in self._cache.items()
            if current_time - timestamp > self.timeout
        ]
