...
class UIManager:
    ...
    def __init__(self, data_manager: DataManager, permission_manager: PermissionManager):
        """
        初始化界面管理器
    ...
    async def _render_preview_page(self, guild_id: int, user: discord.Member) -> discord.Embed:
        """渲染預覽頁面"""
        ...
        embed.set_footer(text=f"活躍度系統 • 預覽面板 • {datetime.now().strftime('%Y-%m-%d')}")
        
        return embed
        
    async def _render_stats_page(self, guild_id: int, user: discord.Member) -> discord.Embed:
        """渲染統計頁面"""
    ...
    async def _render_history_page(self, guild_id: int, user: discord.Member) -> discord.Embed:
        """渲染歷史頁面"""
        embed = discord.Embed(
            title="📜 活躍度歷史記錄",
