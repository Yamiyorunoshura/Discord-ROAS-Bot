...
class DataManager:
    ...
    async def get_rankings(self, guild_id: int, time_range: str = "daily") -> List[Dict]:
        """
        獲取排行榜數據
    ...
    async def fetch_rankings_from_db(self, guild_id: int, time_range: str) -> List[Dict]:
        """
        從數據庫獲取排行榜數據
    ...
    async def get_settings(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            channel_id = next((ch_id for g_id, ch_id in report_channels if g_id == guild_id), None)
    ...
    async def get_stats(self, guild_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # 獲取各種統計數據
            today_rankings = await self.db.get_daily_rankings(today_ymd, guild_id, limit=5)
            yesterday_rankings = await self.db.get_daily_rankings(yesterday_ymd, guild_id, limit=5)
