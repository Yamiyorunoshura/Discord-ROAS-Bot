...
class PageManager:
    ...
    async def _load_stats_data(self, interaction: discord.Interaction):
        ...
        # 獲取統計數據
        today_rankings = await db.get_daily_rankings(today_ymd, interaction.guild.id, limit=3)
        yesterday_rankings = await db.get_daily_rankings(yesterday_ymd, interaction.guild.id, limit=3)
    ...
    def get_page_data(self, page_name: Optional[str] = None) -> Dict[str, Any]:
        """獲取頁面數據"""
