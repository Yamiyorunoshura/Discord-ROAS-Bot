...
# 導入測試模塊
from .activity_test_module import ActivityTestModule, TestReport, TestStatus
...
class ActivityTestRunner:
    ...
    async def run_complete_test_suite(self) -> TestReport:
        ...
        try:
            ...
            functional_test_config = {
                "test_types": ["calculation", "rendering", "settings", "panel", "database"],
                "test_count": 5,
                "timeout": 30
            }
            
            functional_report = await self.test_module.test_real_logic(functional_test_config)
            ...
            for endpoint in performance_endpoints:
                try:
                    performance_data = await self.test_module.test_real_logic_response_time(endpoint)
            ...
            error_scenarios = [
                ...
                {
                    "name": "無效的數據格式",
                    "test_data": {"request_type": "calculate_activity_score", "parameters": "invalid"},
                    "expected_error": "必須是字典格式"
                }
            ]
            
            try:
                error_handling_results = await self.test_module.test_real_logic_error_handling(error_scenarios)
            except Exception as e:
            ...
            self.logger.info(f"完整測試套件執行完成: 總時間 {total_execution_time:.2f}秒")
    ...
    async def run_functional_tests(self) -> TestReport:
        ...
        try:
            ...
            self.logger.info(f"功能測試完成: {test_report.passed_tests}/{test_report.total_tests} 通過")
            return test_report
    ...
    async def run_error_handling_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"錯誤處理測試完成: {error_handling_results.get('passed_error_tests', 0)}/{error_handling_results.get('total_error_tests', 0)} 通過")
    ...
    async def run_coverage_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 分析覆蓋率
            coverage_data = await self.test_module.analyze_test_coverage(test_report.test_results)
            
            # 保存覆蓋率分析結果
            await self._save_coverage_results(coverage_data)
            
            self.logger.info(f"覆蓋率分析完成: 模塊覆蓋率 {coverage_data.get('module_coverage', 0):.1f}%, API覆蓋率 {coverage_data.get('api_coverage', 0):.1f}%")
    ...
    async def _save_test_report(self, test_report: TestReport, report_type: str = "comprehensive"):
        """保存測試報告"""
        try:
    ...
    async def _save_error_handling_results(self, error_handling_results: Dict[str, Any]):
        """保存錯誤處理測試結果"""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
