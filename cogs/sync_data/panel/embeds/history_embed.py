...
async def create_history_embed(cog: "SyncDataCog", guild: discord.Guild, page: int = 0) -> discord.Embed:
    """
    創建同步歷史記錄嵌入
    ...
    try:
        ...
        if total_pages > 1:
            embed.set_footer(text=f"第 {page + 1}/{total_pages} 頁 | 點擊「刷新」更新資料")
        else:
...
async def _get_sync_history(cog: "SyncDataCog", guild_id: int, page: int = 0) -> List[Dict[str, Any]]:
    """
    獲取同步歷史記錄
