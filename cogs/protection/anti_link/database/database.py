...
from ..config.config import DEFAULTS, ERROR_CODES
...
class AntiLinkDatabase:
    ...
    async def init_db(self):
        """初始化資料庫表格"""
        try:
            async with aiosqlite.connect("dbs/anti_link.db") as db:
                # 創建配置表
                await db.execute("""
                ...
                # 創建索引
                await db.execute("CREATE INDEX IF NOT EXISTS idx_config_guild ON config(guild_id)")
                await db.execute("CREATE INDEX IF NOT EXISTS idx_stats_guild ON stats(guild_id)")
                await db.execute("CREATE INDEX IF NOT EXISTS idx_logs_guild ON action_logs(guild_id)")
    ...
    # ───────── 配置管理 ─────────
    async def get_config(self, guild_id: int, key: str, default: str | None = None) -> str | None:
        """
        取得配置值
    ...
    async def update_blacklist_cache(self, domains: Set[str], source: str):
        ...
        try:
            async with aiosqlite.connect("dbs/anti_link.db") as db:
                ...
                await db.commit()
                self.logger.info(f"【反惡意連結】更新黑名單快取: {len(domains)} 個網域來自 {source}")
    ...
    async def get_blacklist_cache(self, source: str | None = None) -> Set[str]:
        ...
        try:
            async with aiosqlite.connect("dbs/anti_link.db") as db:
                if source:
                    async with db.execute(
                        "SELECT domain FROM blacklist_cache WHERE source = ?",
                        (source,)
                    ) as cursor:
                        rows = await cursor.fetchall()
                else:
        ...
        try:
            async with aiosqlite.connect("dbs/anti_link.db") as db:
                ...
                result = await db.execute(
                    "DELETE FROM blacklist_cache WHERE last_seen < ?",
                    (cutoff_date,)
                )
                
                await db.commit()
                self.logger.info(f"【反惡意連結】清理黑名單快取: 刪除 {result.rowcount} 個過期項目")
            ...
            async with aiosqlite.connect("dbs/anti_link.db") as db:
                await db.execute("""
                    ...
                    VALUES (?, ?, 
                        ...
                        ?)
                """, (guild_id, stat_type, guild_id, stat_type, count, dt.datetime.now()))
    ...
    async def cleanup_action_logs(self, guild_id: int, days: int = 30):
        ...
        try:
            async with aiosqlite.connect("dbs/anti_link.db") as db:
                ...
                await db.commit()
                self.logger.info(f"【反惡意連結】清理操作日誌: 刪除 {result.rowcount} 個過期項目")
