...
class AntiExecutableDatabase:
    ...
    def __init__(self, cog):
        ...
        """
        self.cog = cog
        self.db_path = "dbs/anti_executable.db"
    ...
    async def get_config(self, guild_id: int, key: str, default: str = "") -> str:
        ...
        try:
            ...
            error_handler.log_error(exc, f"獲取配置 - {guild_id}:{key}", "CONFIG_GET_ERROR")
            return default
    ...
    async def set_config(self, guild_id: int, key: str, value: str):
        ...
        try:
            ...
            error_handler.log_error(exc, f"設置配置 - {guild_id}:{key}", "CONFIG_SET_ERROR")
    ...
    async def get_all_config(self, guild_id: int) -> Dict[str, str]:
        ...
        try:
            ...
            error_handler.log_error(exc, f"獲取所有配置 - {guild_id}", "CONFIG_GET_ALL_ERROR")
            return {}
        ...
        try:
            async with self._lock:
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    """, (guild_id, stat_type, guild_id, stat_type, count, datetime.now()))
                    await db.commit()
                    
        except Exception as exc:
            error_handler.log_error(exc, f"添加統計 - {guild_id}:{stat_type}", "STATS_ADD_ERROR")
    ...
    async def add_action_log(self, guild_id: int, user_id: int, action: str, details: str = ""):
        ...
        try:
            ...
            error_handler.log_error(exc, f"添加操作日誌 - {guild_id}:{user_id}", "ACTION_LOG_ADD_ERROR")
    
    async def get_action_logs(self, guild_id: int, limit: int = 100) -> List[Dict[str, Any]]:
        """
        獲取操作日誌
        ...
        try:
            ...
            error_handler.log_error(exc, f"獲取操作日誌 - {guild_id}", "ACTION_LOG_GET_ERROR")
            return []
    ...
    async def cleanup_action_logs(self, guild_id: int, days: int = 30):
        ...
        try:
            ...
            error_handler.log_error(exc, f"清理操作日誌 - {guild_id}", "ACTION_LOG_CLEANUP_ERROR")
    
    # ───────── 檔案檢測記錄 ─────────
    async def add_file_detection(self, guild_id: int, user_id: int, filename: str, 
                                file_extension: str, risk_level: str, action_taken: str):
        """
        添加檔案檢測記錄
            ...
            async with self._lock:
                async with aiosqlite.connect(self.db_path) as db:
                    await db.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?)
                    """, (guild_id, user_id, filename, file_extension, risk_level, action_taken, datetime.now()))
                    await db.commit()
                    
        except Exception as exc:
            error_handler.log_error(exc, f"添加檔案檢測記錄 - {guild_id}:{user_id}", "FILE_DETECTION_ADD_ERROR")
    
    async def get_file_detections(self, guild_id: int, limit: int = 100) -> List[Dict[str, Any]]:
        """
        獲取檔案檢測記錄
        ...
        try:
            ...
            error_handler.log_error(exc, f"獲取檔案檢測記錄 - {guild_id}", "FILE_DETECTION_GET_ERROR")
            return []
    
    async def get_detection_stats(self, guild_id: int, days: int = 30) -> Dict[str, Any]:
        """
        獲取檔案檢測統計
        ...
        try:
            async with self._lock:
                async with aiosqlite.connect(self.db_path) as db:
                    ...
                    async with db.execute("""
                        ...
                        LIMIT 10
                    """, (guild_id, cutoff_date)) as cursor:
                        extension_stats = {row[0]: row[1] for row in await cursor.fetchall()}
            ...
            error_handler.log_error(exc, f"獲取檢測統計 - {guild_id}", "DETECTION_STATS_ERROR")
            return {}
    ...
    async def cleanup_file_detections(self, guild_id: int, days: int = 90):
        ...
        try:
            ...
            error_handler.log_error(exc, f"清理檔案檢測記錄 - {guild_id}", "FILE_DETECTION_CLEANUP_ERROR")
