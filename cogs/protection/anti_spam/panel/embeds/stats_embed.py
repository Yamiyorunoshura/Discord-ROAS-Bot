...
async def create_stats_embed(cog: "AntiSpam", guild: discord.Guild) -> discord.Embed:
    ...
    embed.add_field(
        name="⚔️ 處理統計",
        value=f"禁言處理: {timeouts} 次",
        inline=True
    )
    
    # 效率統計
    if total_violations > 0:
        success_rate = (timeouts / total_violations) * 100 if total_violations > 0 else 0
        embed.add_field(
            name="📈 處理效率",
            value=f"處理成功率: {success_rate:.1f}%",
            inline=True
        )
    ...
    embed.set_footer(text=f"統計更新時間：{dt.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    return embed
