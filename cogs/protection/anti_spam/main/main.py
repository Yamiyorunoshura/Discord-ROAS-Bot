...
from ..config.config import DEFAULTS, CONFIG_CATEGORIES, CONFIG_KEY_MAP
...
class AntiSpam(ProtectionCog):
    ...
    # ───────── 檢測邏輯 ─────────
    async def _match_freq_limit(self, guild_id: int, key: Tuple[int, int], now: float) -> bool:
        """檢查頻率限制"""
        try:
            limit_str = await self.get_cfg(guild_id, "spam_freq_limit", str(DEFAULTS["spam_freq_limit"]))
            window_str = await self.get_cfg(guild_id, "spam_freq_window", str(DEFAULTS["spam_freq_window"]))
    ...
    async def _match_identical(self, guild_id: int, key: Tuple[int, int], now: float) -> bool:
        """檢查重複訊息"""
        try:
            limit_str = await self.get_cfg(guild_id, "spam_identical_limit", str(DEFAULTS["spam_identical_limit"]))
            window_str = await self.get_cfg(guild_id, "spam_identical_window", str(DEFAULTS["spam_identical_window"]))
    ...
    async def _match_similar(self, guild_id: int, key: Tuple[int, int], now: float) -> bool:
        """檢查相似訊息"""
        try:
            limit_str = await self.get_cfg(guild_id, "spam_similar_limit", str(DEFAULTS["spam_similar_limit"]))
            window_str = await self.get_cfg(guild_id, "spam_similar_window", str(DEFAULTS["spam_similar_window"]))
            threshold_str = await self.get_cfg(guild_id, "spam_similar_threshold", str(DEFAULTS["spam_similar_threshold"]))
    ...
    async def _match_sticker(self, guild_id: int, key: Tuple[int, int], now: float) -> bool:
        """檢查貼圖濫用"""
        try:
            limit_str = await self.get_cfg(guild_id, "spam_sticker_limit", str(DEFAULTS["spam_sticker_limit"]))
            window_str = await self.get_cfg(guild_id, "spam_sticker_window", str(DEFAULTS["spam_sticker_window"]))
    ...
    async def _handle_violation(self, msg: discord.Message, violations: List[str]):
        ...
        try:
            ...
            # 處理超時
            timeout_str = await self.get_cfg(msg.guild.id, "spam_timeout_minutes", str(DEFAULTS["spam_timeout_minutes"]))
            ...
            # 發送回復訊息
            response_enabled = await self.get_cfg(msg.guild.id, "spam_response_enabled", DEFAULTS["spam_response_enabled"])
            if response_enabled and response_enabled.lower() == "true":
                response_message = await self.get_cfg(msg.guild.id, "spam_response_message", DEFAULTS["spam_response_message"])
                try:
            ...
            # 發送管理員通知
            channel_mention = getattr(msg.channel, 'mention', f"#{getattr(msg.channel, 'name', 'unknown')}")
            notify_text = (
                f"🚫 **反垃圾訊息警報**\n"
            ...
            await self._add_action_log(
                msg.guild.id, msg.author.id, "violation",
                f"違規類型: {', '.join(violations)}, 累計: {violation_count} 次"
            )
            
            logger.info(f"【反垃圾訊息】處理違規: {msg.author.id} - {', '.join(violations)}")
            
        except Exception as exc:
            error_handler.log_error(exc, f"處理違規行為 - {msg.author.id}", "VIOLATION_HANDLER_ERROR")
    ...
    async def spam_panel(self, interaction: discord.Interaction):
        ...
        try:
            ...
            error_handler.log_error(exc, f"開啟設定面板 - {interaction.user.id}", "PANEL_ERROR_532")
        ...
        try:
            ...
            for guild_id in list(self.action_logs.keys()):
                self.action_logs[guild_id] = [
                    log for log in self.action_logs[guild_id]
