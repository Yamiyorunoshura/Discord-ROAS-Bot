# Discord ADR Bot v1.7 - 更新日誌

## [v1.7.0] - 2025-01-18

### 🎉 重大更新 - 活躍度面板系統
- **完整活躍度面板系統**：實現四頁面管理界面
  - 設定頁面：系統參數配置、頻道設定、等級系統自定義
  - 預覽頁面：即時排行榜、個人活躍度查詢、視覺化效果
  - 統計頁面：詳細數據分析、趨勢圖表、性能監控
  - 歷史頁面：歷史記錄管理、數據導出、備份還原
- **分層權限管理系統**：四級權限架構
  - 查看權限：所有成員可查看面板和統計
  - 基本操作權限：所有成員可查看個人活躍度
  - 管理權限：管理員可修改系統設定
...
#### 活躍度面板系統 (`cogs/activity_meter`)
- 新增 `/活躍度面板` 指令，開啟完整管理界面
- 實現設定頁面：播報頻道、計算參數、播報時間、系統開關
- 實現預覽頁面：即時排行榜、個人查詢、時間範圍切換
- 實現統計頁面：用戶分析、頻道排行、趨勢圖表、性能監控
- 實現歷史頁面：歷史記錄、數據導出、清理備份
- 新增權限檢查器：智能權限驗證和訪問控制
- 新增數據渲染器：統一的數據展示和圖表生成
- 新增頁面管理器：流暢的頁面切換和狀態管理
