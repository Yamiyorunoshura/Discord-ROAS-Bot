...
class DocumentationTester:
    ...
    def _validate_api_endpoint(self, method: str, path: str, spec: dict) -> dict[str, Any]:
        """驗證單個 API 端點"""
    ...
    def _calculate_summary(self, report: dict[str, Any]) -> dict[str, Any]:
        ...
        if summary['total_files'] > 0:
            summary['file_success_rate'] = summary['valid_files'] / summary['total_files']

        if summary['total_links'] > 0:
            summary['link_success_rate'] = (summary['total_links'] - summary['broken_links']) / summary['total_links']

        if summary['total_code_blocks'] > 0:
            summary['code_success_rate'] = (summary['total_code_blocks'] - summary['invalid_code_blocks']) / summary['total_code_blocks']

        if summary['api_endpoints'] > 0:
            summary['api_success_rate'] = summary['working_endpoints'] / summary['api_endpoints']
...
def test_markdown_files_valid(doc_tester):
    ...
    assert len(failed_files) == 0, f"以下文件有語法錯誤: {[f['file'] for f in failed_files]}"
