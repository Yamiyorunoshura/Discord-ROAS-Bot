...
class TestAdminPanelIntegration:
    ...
    @pytest.mark.asyncio
    async def test_criteria_manager_integration(self, admin_panel, mock_interaction, sample_achievement):
        """測試條件管理器整合."""
        # 設置模擬返回
        admin_panel.achievement_service.get_achievement_by_id.return_value = sample_achievement
    ...
    @pytest.mark.asyncio
    async def test_achievement_criteria_selection_integration(self, admin_panel, mock_interaction, sample_achievement):
        """測試成就條件選擇整合."""
        from src.cogs.achievement.panel.admin_panel import AchievementCriteriaSelectionView
    ...
    @pytest.mark.asyncio
    async def test_real_admin_service_integration(self, admin_panel, sample_achievement):
        """測試真實管理服務整合."""
        # 設置模擬返回
    ...
    async def test_navigation_integration(self, admin_panel, mock_interaction):
        ...
        # 執行測試 - 導航到成就管理
        await admin_panel.handle_navigation(mock_interaction, AdminPanelState.ACHIEVEMENTS)
    ...
    @pytest.mark.asyncio
    async def test_permission_validation_integration(self, admin_panel, mock_interaction):
        """測試權限驗證整合."""
        # 設置模擬權限檢查
        admin_panel.admin_permission_service.has_admin_permission.return_value = True
        
        # 執行權限檢查
        has_permission = await admin_panel.admin_permission_service.has_admin_permission(
            987654321, 123456789
        )
    ...
    async def test_statistics_caching_integration(self, admin_panel):
        ...
        # 模擬載入統計數據
        admin_panel.achievement_service.get_system_stats = AsyncMock(return_value=mock_stats)
    ...
    @pytest.mark.asyncio
    async def test_full_workflow_integration(self, admin_panel, mock_interaction, sample_achievement):
        """測試完整工作流程整合."""
        # 設置模擬數據
        admin_panel._load_system_stats = AsyncMock(return_value={
            'total_users': 100,
        ...
        admin_panel.achievement_service.get_all_achievements.return_value = [sample_achievement]
        admin_panel.achievement_service.get_achievement_by_id.return_value = sample_achievement
