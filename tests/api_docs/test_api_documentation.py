...
class APIDocumentationTester:
    ...
    def _test_single_endpoint(self, method: str, path: str, spec: dict) -> dict[str, Any]:
        """測試單個 API 端點"""
            ...
            for path, methods in spec.get('paths', {}).items():
                for method, details in methods.items():
                    if method.upper() == 'GET':  # 主要測試 GET 端點
                        result = self._validate_response_schema(
                            path, details, spec
                        )
                        if result:
                            results.append(result)
    ...
    def _validate_response_schema(self, path: str, spec: dict, full_spec: dict) -> dict[str, Any]:
        """驗證回應 schema"""
        full_url = f"{self.api_base_url}{path}"
    ...
    def test_api_documentation_completeness(self) -> dict[str, Any]:
        ...
        try:
            ...
            completeness_rate = documented_endpoints / total_endpoints if total_endpoints > 0 else 0
...
def test_api_endpoints_are_accessible(api_tester):
    ...
    inaccessible_endpoints = [
        r for r in results
        if not r.get('accessible', False) and 'error' not in r
    ]

    if inaccessible_endpoints:
        endpoints_info = [
            f"{ep['method']} {ep['path']}"
            for ep in inaccessible_endpoints
        ]
