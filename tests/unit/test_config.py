...
class TestSettings:
    ...
    def test_settings_environment_handling(self):
        ...
        with patch.dict(
            os.environ,
            {"TOKEN": TEST_TOKEN, "ENVIRONMENT": "production", "DEBUG": "true"},
        ):
            settings = Settings()
...
class TestDatabaseSettings:
    ...
    def test_database_settings_with_env_vars(self):
        """測試環境變數覆蓋資料庫設置."""
        with patch.dict(os.environ, {"DB_POOL_SIZE": "20", "DB_QUERY_TIMEOUT": "60"}):
            db_settings = DatabaseSettings()
...
class TestCacheSettings:
    ...
    def test_cache_settings_with_env_vars(self):
        """測試環境變數覆蓋快取設置."""
        with patch.dict(os.environ, {"CACHE_TTL": "3600"}):
            cache_settings = CacheSettings()
...
class TestConfigurationValidation:
    ...
    def test_invalid_environment(self):
        """測試無效環境值."""
        with (
            patch.dict(os.environ, {"TOKEN": TEST_TOKEN, "ENVIRONMENT": "invalid_env"}),
