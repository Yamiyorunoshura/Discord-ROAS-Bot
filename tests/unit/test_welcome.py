...
from src.cogs.welcome.main.cache import WelcomeCache

# 導入要測試的模組
from src.cogs.welcome.main.main import WelcomeCog
from src.cogs.welcome.main.renderer import (
    AvatarDownloader,
    DefaultTemplate,
    FontManager,
    LayoutCalculator,
...
class TestAvatarDownloader:
    ...
    async def test_get_avatar_success(self, downloader):
        ...
        with (
            ...
            patch.object(downloader, "process_avatar") as mock_process,
        ):
            test_avatar = Image.new("RGBA", (256, 256), (255, 0, 0, 255))
    ...
    async def test_get_avatar_failure_fallback(self, downloader):
        ...
        # 模擬下載失敗
        with (
            patch.object(downloader, "download_with_retry", return_value=None),
            patch.object(downloader, "get_default_avatar") as mock_default,
        ):
...
class TestWelcomeCog:
    ...
    def welcome_cog(self, mock_bot):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            # 創建測試用的背景目錄
            bg_dir = Path(temp_dir) / "backgrounds"
            ...
            # 模擬配置
            with (
                patch("cogs.welcome.main.main.WELCOME_BG_DIR", bg_dir),
                patch("config.WELCOME_DB_PATH", db_path),
            ):
    ...
    async def test_generate_welcome_image_no_cache(self, welcome_cog):
        ...
        with (
            ...
            patch.object(welcome_cog.db, "get_settings", return_value=settings),
            patch.object(welcome_cog.db, "get_background_path", return_value=None),
        ):
            result = await welcome_cog._generate_welcome_image(12345, mock_member)
            assert result is not None
            assert isinstance(result, io.BytesIO)
