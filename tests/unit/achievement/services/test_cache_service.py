...
async def cache_service():
    """測試用快取服務."""
    with patch('src.cogs.achievement.services.cache_service.AchievementCacheStrategy'), \
...
class TestCacheServiceInitialization:
    ...
    async def test_service_initialization(self):
        """測試服務初始化."""
        with patch('src.cogs.achievement.services.cache_service.AchievementCacheStrategy') as mock_strategy, \
             patch('src.cogs.achievement.services.cache_service.CacheInvalidationManager') as mock_invalidation, \
             patch('src.cogs.achievement.services.cache_service.PerformanceOptimizer') as mock_optimizer, \
             patch('src.cogs.achievement.services.cache_service.CacheConfigManager') as mock_config:
...
class TestBasicCacheOperations:
    ...
    async def test_get_cached_achievement(self, cache_service, sample_achievement):
        ...
        # 模擬快取策略返回資料
        cache_service._strategy.get_cached_data = AsyncMock(return_value=sample_achievement)
    ...
    async def test_cache_achievement(self, cache_service, sample_achievement):
        ...
        cache_service._strategy.cache_data.assert_called_once_with(
            f"achievement:{sample_achievement.id}",
            sample_achievement
        )

    @pytest.mark.asyncio
    async def test_get_cached_user_achievements(self, cache_service, sample_user_achievement):
        """測試取得快取用戶成就."""
        user_id = 123456789
    ...
    @pytest.mark.asyncio
    async def test_cache_user_achievements(self, cache_service, sample_user_achievement):
        """測試快取用戶成就列表."""
        user_id = 123456789
        achievements = [sample_user_achievement]
...
class TestPerformanceMonitoring:
    ...
    async def test_get_cache_statistics(self, cache_service):
        ...
        # 模擬效能優化器
        cache_service._performance_optimizer.get_performance_stats = AsyncMock(return_value=mock_stats)
    ...
    async def test_get_cache_health_status(self, cache_service):
        ...
        # 模擬效能優化器
        cache_service._performance_optimizer.get_health_status = AsyncMock(return_value=mock_health)
...
class TestErrorHandling:
    ...
    async def test_cache_operation_error_handling(self, cache_service):
        """測試快取操作錯誤處理."""
        # 模擬快取策略拋出異常
        cache_service._strategy.get_cached_data = AsyncMock(side_effect=Exception("快取錯誤"))
...
class TestCacheWarmup:
    ...
    async def test_scheduled_warmup(self, cache_service):
        """測試定時預熱."""
        with patch.object(cache_service, 'warmup_popular_achievements') as mock_popular, \
