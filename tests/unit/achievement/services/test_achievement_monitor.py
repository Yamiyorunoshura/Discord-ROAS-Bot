...
class TestMonitorInitialization:
    ...
    async def test_monitor_initialization_with_custom_config(self, mock_cache_manager):
        """測試使用自訂配置初始化監控器."""
        custom_thresholds = {
            MetricType.QUERY_TIME: 50.0,
            MetricType.MEMORY_USAGE: 0.9
        }
...
class TestMetricCollection:
    ...
    @pytest.mark.asyncio
    async def test_record_cache_performance(self, performance_monitor, mock_cache_manager):
        """測試記錄快取效能."""
        ...
        # 驗證快取指標被記錄
        hit_rate_metrics = performance_monitor._get_recent_metrics(MetricType.CACHE_HIT_RATE)
    ...
    async def test_record_memory_usage(self, performance_monitor):
        ...
        with patch('psutil.virtual_memory') as mock_memory:
            ...
            # 驗證記憶體指標被記錄
            memory_metrics = performance_monitor._get_recent_metrics(MetricType.MEMORY_USAGE)
    ...
    async def test_record_database_connections(self, performance_monitor):
        """測試記錄資料庫連線狀態."""
        with patch.object(performance_monitor, '_get_db_connection_count', return_value=15):
            await performance_monitor.record_database_connections()

            # 驗證資料庫連線指標被記錄
            db_metrics = performance_monitor._get_recent_metrics(MetricType.DATABASE_CONNECTIONS)
    ...
    async def test_record_error_rate(self, performance_monitor):
        """測試記錄錯誤率."""
        # 記錄一些操作和錯誤
        await performance_monitor.record_operation_result("get_achievement", success=True)
        await performance_monitor.record_operation_result("get_achievement", success=True)
        await performance_monitor.record_operation_result("get_achievement", success=False)
...
class TestAlertSystem:
    ...
    async def test_memory_usage_alert(self, performance_monitor):
        ...
        with patch('psutil.virtual_memory') as mock_memory:
            ...
            # 驗證產生了記憶體使用量警報
            memory_alerts = [a for a in alerts if a.metric_type == MetricType.MEMORY_USAGE]
...
class TestErrorHandling:
    ...
    async def test_alert_system_error_handling(self, performance_monitor):
        """測試警報系統錯誤處理."""
        # 模擬警報處理失敗
        with patch.object(performance_monitor, '_send_alert', side_effect=Exception("警報發送失敗")):
            # 記錄觸發警報的指標
    ...
    @pytest.mark.asyncio
    async def test_cache_monitoring_failure_fallback(self, performance_monitor, mock_cache_manager):
        """測試快取監控失敗時的回退機制."""
        # 模擬快取管理器異常
