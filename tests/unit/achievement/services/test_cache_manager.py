...
class TestL2FileCache:
    ...
    async def test_l2_cache_file_operations(self, cache_manager):
        ...
        # 驗證檔案建立
        expected_file = cache_manager._get_l2_cache_path(CacheType.ACHIEVEMENT, cache_key)
...
class TestCacheInvalidation:
    ...
    async def test_invalidate_by_pattern(self, cache_manager):
        ...
        # 驗證失效結果
        assert await cache_manager.get(CacheType.USER_ACHIEVEMENT, "user:123:achievements") is None
        assert await cache_manager.get(CacheType.USER_ACHIEVEMENT, "user:123:progress") is None
        assert await cache_manager.get(CacheType.USER_ACHIEVEMENT, "user:456:achievements") == test_data
        assert await cache_manager.get(CacheType.USER_ACHIEVEMENT, "achievement:1") == test_data
