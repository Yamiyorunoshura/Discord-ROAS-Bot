...
class TestActivityCalculator:
    ...
    def calculator(self):
        """建立測試用計算器"""
        from src.cogs.activity_meter.main.calculator import ActivityCalculator
...
class TestActivityDatabase:
    ...
    async def activity_db(self, activity_test_db):
        """建立測試用活躍度資料庫"""
        from src.cogs.activity_meter.database.database import ActivityDatabase
...
class TestActivityRenderer:
    ...
    def renderer(self):
        """建立測試用渲染器"""
        from src.cogs.activity_meter.main.renderer import ActivityRenderer
...
def test_config_validation():
    """測試配置驗證"""
    from src.cogs.activity_meter.config import config
