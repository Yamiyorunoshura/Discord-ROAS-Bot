...
class AvatarDownloader:
    ...
    def __init__(self):
        self.session = None
        self.cache = {}
        self.retry_config = {"max_retries": 3, "backoff_factor": 2, "timeout": 10}
...
class LayoutCalculator:
    ...
    def __init__(self):
        self.aspect_ratios = {"16:9": (16, 9), "4:3": (4, 3), "1:1": (1, 1)}
...
class WelcomeRenderer:
    ...
    async def generate_welcome_image(
        ...
        try:
            ...
            avatar = avatar.resize(
                (layout.avatar_size, layout.avatar_size), Image.Resampling.LANCZOS
            )
    ...
    def _create_canvas(
        self, layout: LayoutConfig, template: WelcomeTemplate, bg_path: str | None
    ) -> Image.Image:
        """
        創建畫布
    ...
    def _draw_text(
        ...
        draw.text(
            (
                layout.title_position[0] + shadow_offset,
                layout.title_position[1] + shadow_offset,
            ),
            title,
            font=title_font,
        ...
        draw.text(
            layout.title_position, title, font=title_font, fill=template.text_color
        )

        # 繪製描述
        description = settings.get("description", "歡迎加入!").format(
        ...
        draw.text(
            (
                layout.description_position[0] + shadow_offset,
                layout.description_position[1] + shadow_offset,
            ),
            description,
            font=desc_font,
            fill=(0, 0, 0, 128),  # 半透明黑色陰影
        ...
        for element in layout.decorative_elements:
            ...
            if element_type == "line":
                ...
                if element.get("filled", False):
                    ...
                    draw.ellipse(
                        bbox, outline=element["color"], width=element.get("width", 1)
                    )
    ...
    def render_message(
        ...
        try:
            # 基本變數替換
            msg = msg_template.format(member=member, guild=guild, channel=channel)
