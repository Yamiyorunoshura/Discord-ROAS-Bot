...
class BaseModal(discord.ui.Modal):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.debug(f"創建對話框: {self.__class__.__name__}")

    async def _after_update(
        self, interaction: discord.Interaction, success_message: str = "✅ 設定已更新"
    ):
        """
        更新設定後的共用處理
        ...
        try:
            ...
            if self.panel_msg and self.panel_msg.guild:
                try:
                    ...
                    # 建立新的 Embed
                    embed = await build_settings_embed(
                        self.cog, self.panel_msg.guild, settings
                    )
                    ...
                    logger.error(f"更新設定面板失敗: {panel_exc}")

            logger.debug(f"對話框處理完成: {self.__class__.__name__}")
...
class SetChannelModal(BaseModal):
    ...
    def __init__(self, cog: "WelcomeCog", panel_msg: discord.Message | None):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not channel:
            await interaction.response.send_message(
                "❌ 找不到指定的頻道,請確認頻道名稱或 ID 是否正確", ephemeral=True
            )
            return
        ...
        if not (permissions.send_messages and permissions.attach_files):
            await interaction.response.send_message(
                "❌ 機器人在該頻道沒有發送訊息或附加檔案的權限", ephemeral=True
            )
            return
...
class SetTitleModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await self.cog.db.update_setting(
            interaction.guild_id, "title", self.title_txt.value
        )
...
class SetDescModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "description", self.desc_txt.value
        )
...
class SetMsgModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "message", self.msg_txt.value
        )
...
class SetAvatarXModal(BaseModal):
    ...
    def __init__(self, cog: "WelcomeCog", panel_msg: discord.Message | None):
        ...
        self.x_txt = discord.ui.TextInput(
            label="X 座標(像素)", placeholder="例如:30", required=True, max_length=4
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self.x_txt.value.isdigit():
            await interaction.response.send_message(
                "❌ X 座標必須是數字", ephemeral=True
            )
            return

        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "avatar_x", int(self.x_txt.value)
        )
...
class SetAvatarYModal(BaseModal):
    ...
    def __init__(self, cog: "WelcomeCog", panel_msg: discord.Message | None):
        ...
        self.y_txt = discord.ui.TextInput(
            label="Y 座標(像素)", placeholder="例如:80", required=True, max_length=4
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self.y_txt.value.isdigit():
            await interaction.response.send_message(
                "❌ Y 座標必須是數字", ephemeral=True
            )
            return

        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "avatar_y", int(self.y_txt.value)
        )
...
class SetTitleYModal(BaseModal):
    ...
    def __init__(self, cog: "WelcomeCog", panel_msg: discord.Message | None):
        ...
        self.y_txt = discord.ui.TextInput(
            label="Y 座標(像素)", placeholder="例如:60", required=True, max_length=4
        )
        self.add_item(self.y_txt)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "title_y", int(self.y_txt.value)
        )
...
class SetDescYModal(BaseModal):
    ...
    def __init__(self, cog: "WelcomeCog", panel_msg: discord.Message | None):
        ...
        self.y_txt = discord.ui.TextInput(
            label="Y 座標(像素)", placeholder="例如:120", required=True, max_length=4
        )
        self.add_item(self.y_txt)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "description_y", int(self.y_txt.value)
        )
...
class SetTitleFontSizeModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if font_size < MIN_FONT_SIZE or font_size > MAX_FONT_SIZE:
            await interaction.response.send_message(
                f"❌ 字體大小必須在 {MIN_FONT_SIZE} 到 {MAX_FONT_SIZE} 之間",
                ephemeral=True,
            )
            return
        ...
        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "title_font_size", font_size
        )
...
class SetDescFontSizeModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 更新設定
        await self.cog.db.update_setting(
            interaction.guild_id, "desc_font_size", font_size
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.size_txt.value.strip():
            ...
            if avatar_size < MIN_AVATAR_SIZE or avatar_size > MAX_AVATAR_SIZE:
                await interaction.response.send_message(
                    f"❌ 頭像大小必須在 {MIN_AVATAR_SIZE} 到 {MAX_AVATAR_SIZE} 之間",
                    ephemeral=True,
                )
                return
            ...
            # 更新設定
            await self.cog.db.update_setting(
                interaction.guild_id, "avatar_size", avatar_size
            )
        else:
            # 使用預設大小(設為 NULL)
