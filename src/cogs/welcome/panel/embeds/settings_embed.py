...
async def build_settings_embed(
    cog: "WelcomeCog", guild: discord.Guild, settings: dict[str, Any]
) -> discord.Embed:
    """
    建立設定面板 Embed
    ...
    # 添加設定資訊
    embed.add_field(name="📺 歡迎頻道", value=channel_name, inline=True)

    embed.add_field(
        name="📝 圖片標題", value=settings.get("title", "未設定"), inline=True
    )

    embed.add_field(
        name="📄 圖片內容", value=settings.get("description", "未設定"), inline=True
    )

    embed.add_field(
        name="💬 歡迎訊息", value=settings.get("message", "未設定"), inline=False
    )
    ...
    embed.add_field(name="🖼️ 頭像大小", value=avatar_size_text, inline=True)
