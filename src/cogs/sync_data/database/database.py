...
# 服務接口定義
# ────────────────────────────
class ISyncDataDatabase(Protocol):
    """資料同步資料庫服務接口"""
    ...
    async def insert_or_replace_role(self, role: discord.Role) -> None:
        """插入或更新角色資料"""
        ...

    async def insert_or_replace_channel(
    ...
    async def get_last_sync_record(self, guild_id: int) -> dict[str, Any] | None:
        """獲取最後一次同步記錄"""
        ...
...
class SyncDataDatabaseService:
    ...
    def __init__(self, bot: commands.Bot | None = None):
        ...
        Args:
            bot: Discord 機器人實例(可選,用於向後相容)
        """
        self.bot = bot
        self.db_path = str(_settings.get_database_path("sync_data"))
    ...
    async def fetchone(self, query: str, params: tuple = ()) -> dict[str, Any | None]:
        ...
        try:
            ...
            async with pool.get_connection_context(self.db_path) as conn:
                ...
                if row:
                    return dict(
                        zip([desc[0] for desc in cursor.description], row, strict=False)
                    )
                return None
        except Exception as exc:
    ...
    async def validate_data_integrity(self, guild_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            result["roles_count"] = (
                await self.fetchval(
                    "SELECT COUNT(*) FROM roles WHERE guild_id = ?", (guild_id,)
                )
                or 0
            )
            ...
            result["channels_count"] = (
                await self.fetchval(
                    "SELECT COUNT(*) FROM channels WHERE guild_id = ?", (guild_id,)
                )
                or 0
            )
...
# 向後相容性支援
# ────────────────────────────
class SyncDataDatabase(SyncDataDatabaseService):
    """向後相容的資料庫類別名稱"""
