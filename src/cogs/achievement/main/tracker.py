...
class AchievementEventListener(commands.Cog):
    ...
    async def initialize(
        self, achievement_service: AchievementService, database_pool: DatabasePool
    ) -> None:
        """初始化事件監聽器.
        ...
        try:
            ...
            self.event_processor = EventDataProcessor(
                batch_size=50, batch_timeout=5.0, max_memory_events=1000
            )
    ...
    async def _handle_achievement_event(self, event: Event) -> None:
        ...
        try:
            ...
            if processing_time > MAX_EVENT_PROCESSING_TIME_MS:
                logger.warning(
    ...
    async def _persist_event_batch(
        self, batch_events: list[AchievementEventData]
    ) -> None:
        """持久化事件批次.
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        try:
            ...
            event_data = {
                ...
                "account_age_days": (time.time() - member.created_at.timestamp())
                / 86400,
                "is_bot": member.bot,
            }
    ...
    def get_event_stats(self) -> dict[str, Any]:
        ...
        stats = {
            **self._event_stats,
            ...
            "success_rate": (
                self._event_stats["processed_events"]
                / self._event_stats["total_events"]
                if self._event_stats["total_events"] > 0
                else 1.0
            ),
        }
    ...
    async def _trigger_achievement_progress_updates(self, events: list) -> None:
        ...
        try:
            ...
            for user_id, user_event_list in user_events.items():
                try:
                    await self._update_user_achievement_progress(
                        user_id, user_event_list
                    )
                except Exception as e:
    ...
    async def _update_user_achievement_progress(
        self, user_id: int, events: list
    ) -> None:
        """更新單個用戶的成就進度.
        ...
        try:
            ...
            if hasattr(self, "progress_tracker") and self.progress_tracker:
                ...
                # 記錄警告但不阻止處理
                logger.warning(
                    f"[成就進度更新]進度追蹤器不可用,跳過用戶 {user_id} 的進度更新"
                )
    ...
    async def cleanup(self) -> None:
        ...
        try:
            ...
            if self.event_processor:
                ...
                if pending_events:
                    logger.info(
                        f"[成就事件監聽器]清理時處理剩餘事件: {len(pending_events)} 個"
                    )
                    # 持久化剩餘事件
