...
class EventDataProcessor:
    ...
    def __init__(
        ...
        # 資料標準化規則
        self._standardization_rules: dict[
    ...
    def _setup_default_standardization_rules(self) -> None:
        ...
        def standardize_message_event(data: dict[str, Any]) -> dict[str, Any]:
            ...
            # 標準化數值型別
            standardized["content_length"] = max(
                0, int(standardized.get("content_length", 0))
            )
            standardized["mention_count"] = max(
                0, int(standardized.get("mention_count", 0))
            )
        ...
        def standardize_member_event(data: dict[str, Any]) -> dict[str, Any]:
            ...
            if "roles_count" in standardized:
                standardized["roles_count"] = max(
                    0, int(standardized.get("roles_count", 0))
                )
    ...
    def add_standardization_rule(
        self, event_type: str, rule_func: Callable[[dict[str, Any]], dict[str, Any]]
    ) -> None:
        """新增資料標準化規則.
    ...
    async def process_batch(
        ...
        logger.debug(
            f"[事件資料處理器]批次處理完成: {len(processed_events)}/{len(events)} 事件通過"
        )
    ...
    async def add_to_batch(
        self, event_data: dict[str, Any]
    ) -> list[AchievementEventData] | None:
        """新增事件到批次處理佇列.
        ...
        async with self._batch_lock:
            ...
            # 檢查是否需要處理批次
            should_process_batch = len(self._pending_events) >= self.batch_size or (
    ...
    async def _structure_event_data(
        self, event_data: dict[str, Any]
    ) -> AchievementEventData | None:
        """結構化事件資料.
    ...
    async def _standardize_event_data(
        self, event: AchievementEventData
    ...
    async def _validate_event_data(self, _event: AchievementEventData) -> bool:
        """驗證事件資料.
            ...
            **self._stats,
            ...
            "success_rate": (
                (
                    ...
                    - self._stats["filtered_out"]
                    - self._stats["validation_errors"]
                )
                / self._stats["total_processed"]
    ...
    async def flush_pending_events(self) -> list[AchievementEventData]:
        ...
        async with self._batch_lock:
            if self._pending_events:
                ...
                self._last_batch_time = time.time()
                logger.info(
                    f"[事件資料處理器]強制處理待處理事件: {len(batch_events)} 個"
                )
                return batch_events
            return []
