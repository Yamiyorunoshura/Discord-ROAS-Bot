...
class AchievementCog(BaseCog):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._service_container = AchievementServiceContainer(
                database_pool, self.bot
            )
            ...
            self.admin_permission_service = (
                self._service_container.admin_permission_service
            )
    ...
    async def _initialize_notification_system(self) -> None:
        ...
        try:
            # 導入通知整合模組
            initialize_notification_integration = _import_notification_integration()
    ...
    async def achievement_command(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            # 導入並建立面板
            AchievementPanel = _import_achievement_panel()
    ...
    @app_commands.command(
        name="成就管理", description="開啟成就系統管理面板(僅限管理員)"
    )
    async def achievement_admin_command(self, interaction: discord.Interaction) -> None:
        ...
        try:
            ...
            # 權限檢查通過,創建並啟動管理面板
            AdminPanel = _import_admin_panel()
            ...
            logger.info(f"[成就管理]管理員 {interaction.user.id} 成功存取管理面板")
    ...
    async def _cleanup_notification_system(self) -> None:
        """清理通知系統整合."""
        try:
            get_notification_manager = _import_notification_manager()
