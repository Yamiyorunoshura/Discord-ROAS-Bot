...
class RevokeAchievementView(ui.View):
    ...
    async def create_user_achievements_embed(self) -> discord.Embed:
        ...
        try:
            ...
            achievement_list = []
            for i, (user_ach, achievement) in enumerate(page_achievements, start_idx + 1):
                # 獲取分類資訊
                try:
            ...
            if len(self.user_achievements) > self.items_per_page:
                ...
                embed.set_footer(text=f"頁面 {self.current_page + 1}/{total_pages} | 選擇成就後點擊「撤銷」按鈕")
            else:
...
class RevokeConfirmationView(ui.View):
    ...
    def __init__(self, admin_panel: AdminPanel, user_data: dict[str, Any], achievement, user_achievement):
        """初始化撤銷確認視圖."""
    ...
    async def confirm_revoke_button(self, interaction: discord.Interaction, button: ui.Button):
        """確認撤銷（需要二次確認）."""
        try:
            # 顯示二次確認模態框
            ...
            await interaction.response.send_message("❌ 顯示確認對話框時發生錯誤", ephemeral=True)
    ...
    async def _execute_revoke(self, reason: str, interaction: discord.Interaction):
        ...
        try:
            ...
            async with AchievementServiceContainer(pool, self.admin_panel.bot) as container:
                ...
                # 執行撤銷操作
                success, message = await user_admin_service.revoke_achievement_from_user(
...
class RevokeResultView(ui.View):
    ...
    async def continue_revoke_button(self, interaction: discord.Interaction, button: ui.Button):
        """繼續撤銷其他成就."""
        try:
            revoke_view = RevokeAchievementView(self.admin_panel, self.user_data)
            embed = await revoke_view.create_user_achievements_embed()
