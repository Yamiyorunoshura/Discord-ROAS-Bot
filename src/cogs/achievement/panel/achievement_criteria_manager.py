...
class AchievementCriteriaManager:
    ...
    async def start_criteria_editor(self, interaction: discord.Interaction, achievement_id: int):
        """啟動條件編輯器.
    ...
    async def save_criteria(self) -> bool:
        ...
        try:
            ...
            # 調用服務保存
            success = await self.achievement_service.update_achievement(self.current_achievement)
...
class CriteriaEditorView(ui.View):
    ...
    async def message_count_criteria(self, interaction: discord.Interaction, button: ui.Button):
        """設置訊息數量條件."""
    ...
    async def keyword_criteria(self, interaction: discord.Interaction, button: ui.Button):
        """設置關鍵字條件."""
        modal = KeywordCriteriaModal(self.criteria_manager)
        await interaction.response.send_modal(modal)

    @ui.button(label="時間條件", style=discord.ButtonStyle.primary, emoji="⏰")
    async def time_criteria(self, interaction: discord.Interaction, button: ui.Button):
        """設置時間條件."""
        modal = TimeCriteriaModal(self.criteria_manager)
        await interaction.response.send_modal(modal)

    @ui.button(label="複合條件", style=discord.ButtonStyle.secondary, emoji="🔗")
    async def complex_criteria(self, interaction: discord.Interaction, button: ui.Button):
        """設置複合條件."""
    ...
    async def preview_and_save(self, interaction: discord.Interaction, button: ui.Button):
        """預覽並保存條件."""
        try:
    ...
    async def _create_preview_embed(self) -> discord.Embed:
        ...
        embed = StandardEmbedBuilder.create_info_embed(
            "👀 條件預覽",
            f"**成就**: {self.criteria_manager.current_achievement.name}"
        )
        ...
        if criteria:
            ...
            for key, value in criteria.items():
                if key == "target_value":
                    ...
                    preview_text.append(f"• 關鍵字: {', '.join(value) if isinstance(value, list) else value}")
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if self.time_window.value.strip():
                self.criteria_manager.current_criteria["time_window"] = self.time_window.value.strip()
...
class KeywordCriteriaModal(ui.Modal):
    ...
    def __init__(self, criteria_manager: AchievementCriteriaManager):
        ...
        self.keywords = ui.TextInput(
            ...
            default=", ".join(current_keywords) if isinstance(current_keywords, list) else str(current_keywords),
    ...
    async def on_submit(self, interaction: discord.Interaction):
        """提交關鍵字條件."""
        try:
            keywords = [kw.strip() for kw in self.keywords.value.split(",") if kw.strip()]
...
class TimeCriteriaModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 更新條件
            self.criteria_manager.current_criteria["consecutive_days"] = consecutive_days
            self.criteria_manager.current_criteria["daily_activity_type"] = activity_type
...
class ComplexCriteriaView(ui.View):
    ...
    async def and_logic(self, interaction: discord.Interaction, button: ui.Button):
        """設置 AND 邏輯條件."""
        modal = ComplexCriteriaModal(self.criteria_manager, "AND")
        await interaction.response.send_modal(modal)

    @ui.button(label="OR 邏輯", style=discord.ButtonStyle.secondary, emoji="🔀")
    async def or_logic(self, interaction: discord.Interaction, button: ui.Button):
        """設置 OR 邏輯條件."""
...
class ComplexCriteriaModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            self.criteria_manager.current_criteria["complex_description"] = self.condition_description.value
...
class SaveConfirmationView(ui.View):
    ...
    async def confirm_save(self, interaction: discord.Interaction, button: ui.Button):
        """確認保存條件."""
        try:
            await interaction.response.defer(ephemeral=True)
    ...
    async def cancel_save(self, interaction: discord.Interaction, button: ui.Button):
        """取消保存."""
        embed = StandardEmbedBuilder.create_info_embed(
            "🚫 已取消",
            "條件設置已取消，未進行任何更改"
