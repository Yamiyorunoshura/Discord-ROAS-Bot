...
class AchievementRepository(BaseRepository):
    ...
    def _row_to_dict(self, row: Any, columns: list[str] | None = None) -> dict[str, Any]:
        """將資料庫行物件轉換為字典.
        ...
        # 處理元組或列表（需要 columns 參數）
        if hasattr(row, '__iter__') and not isinstance(row, str | bytes):
            if columns:
                return dict(zip(columns, row, strict=False))
            else:
                # 如果沒有 columns，嘗試將其視為序列對
                try:
                    return dict(row)
                except (TypeError, ValueError):
                    # 如果無法轉換，返回空字典
    ...
    async def get_category_by_id(self, category_id: int) -> AchievementCategory | None:
        ...
        Returns:
            成就分類物件或 None
        """
        query = QueryBuilder("achievement_categories").select().where("id", "=", category_id)
        sql, params = query.to_select_sql()

        row = await self.execute_query(sql, params, fetch_one=True)
        if row:
            columns = ['id', 'name', 'description', 'display_order', 'icon_emoji', 'created_at', 'updated_at']
    ...
    async def list_categories(self, active_only: bool = True) -> list[AchievementCategory]:
        ...
        """
        query = (QueryBuilder("achievement_categories")
                .select()
    ...
    async def get_achievement_by_id(self, achievement_id: int) -> Achievement | None:
        ...
        if row:
            ...
            columns = [
                ...
                'points', 'badge_url', 'role_reward', 'is_hidden', 'is_active', 'created_at', 'updated_at'
            ]
            row_dict = self._row_to_dict(row, columns)
    ...
    async def get_user_achievement_by_id(self, record_id: int) -> UserAchievement | None:
        """根據記錄 ID 取得用戶成就.
    ...
    async def has_user_achievement(self, user_id: int, achievement_id: int) -> bool:
        ...
        Returns:
            True 如果已獲得，否則 False
        """
        query = (QueryBuilder("user_achievements")
                .select("1")
    ...
    async def get_user_achievements(
        ...
        Returns:
            (用戶成就記錄, 成就詳情) 的元組列表
        """
        query = (QueryBuilder("user_achievements", "ua")
                .select(
    ...
    async def mark_achievement_notified(self, user_id: int, achievement_id: int) -> bool:
        """標記成就通知已發送.
    ...
    async def get_user_achievement_stats(self, user_id: int) -> dict[str, Any]:
        ...
        """
        # 總成就數量
        total_query = QueryBuilder("user_achievements").count().where("user_id", "=", user_id)
        ...
        # 總點數
        points_query = (QueryBuilder("user_achievements", "ua")
        ...
        # 按分類統計
        category_query = (QueryBuilder("user_achievements", "ua")
        ...
        category_rows = await self.execute_query(category_sql, category_params, fetch_all=True)
    ...
    async def update_progress(
        ...
        if existing_progress:
            ...
            query = (QueryBuilder("achievement_progress")
                    .update(updates)
                    .where("user_id", "=", user_id)
                    .where("achievement_id", "=", achievement_id))
            ...
            if progress_data is not None:
                insert_data["progress_data"] = json.dumps(progress_data, ensure_ascii=False)
        ...
        if current_value >= target_value:
            # 自動頒發成就（如果尚未獲得）
            if not await self.has_user_achievement(user_id, achievement_id):
                await self.award_achievement(user_id, achievement_id)

        return await self.get_user_progress(user_id, achievement_id)

    async def get_user_progress(self, user_id: int, achievement_id: int) -> AchievementProgress | None:
        """取得用戶特定成就的進度.
    ...
    async def get_global_achievement_stats(self) -> dict[str, Any]:
        ...
        total_achievements_row = await self.execute_query(total_achievements_sql, fetch_one=True)
        total_achievements = total_achievements_row[0] if total_achievements_row else 0

        # 活躍成就數量
        active_achievements_query = QueryBuilder("achievements").count().where("is_active", "=", True)
        active_achievements_sql, active_params = active_achievements_query.to_select_sql()
        active_achievements_row = await self.execute_query(active_achievements_sql, active_params, fetch_one=True)
        active_achievements = active_achievements_row[0] if active_achievements_row else 0
        ...
        total_user_achievements_row = await self.execute_query(total_user_achievements_sql, fetch_one=True)
        total_user_achievements = total_user_achievements_row[0] if total_user_achievements_row else 0

        # 獨特用戶數
        unique_users_query = QueryBuilder("user_achievements").select("COUNT(DISTINCT user_id) as count")
    ...
    async def get_popular_achievements(self, limit: int = 10) -> list[tuple[Achievement, int]]:
        ...
        Returns:
            (成就, 獲得人數) 的元組列表
        """
        query = (QueryBuilder("achievements", "a")
                .select(
                    "a.id", "a.name", "a.description", "a.category_id", "a.type", "a.criteria",
                    "a.points", "a.badge_url", "a.is_active", "a.created_at", "a.updated_at",
    ...
    async def get_notification_preferences(
        ...
        Returns:
            用戶通知偏好或 None
        """
        query = (QueryBuilder("notification_preferences")
                .select("*")
                .where("user_id", "=", user_id)
        ...
        if row:
            ...
            columns = [
                'id', 'user_id', 'guild_id', 'dm_notifications', 'server_announcements',
            ...
            if row_dict.get('notification_types'):
                row_dict['notification_types'] = json.loads(row_dict['notification_types'])
            else:
    ...
    async def get_notification_preferences_by_id(self, preference_id: int) -> NotificationPreference | None:
        """根據 ID 取得通知偏好.
    ...
    async def get_global_notification_settings(
        ...
        Returns:
            全域通知設定或 None
        """
        query = (QueryBuilder("global_notification_settings")
                .select("*")
                .where("guild_id", "=", guild_id))
        ...
        if row:
            columns = [
                'id', 'guild_id', 'announcement_channel_id', 'announcement_enabled',
                'rate_limit_seconds', 'important_achievements_only', 'created_at', 'updated_at'
            ]
            row_dict = self._row_to_dict(row, columns)
    ...
    async def get_global_notification_settings_by_id(self, settings_id: int) -> GlobalNotificationSettings | None:
        """根據 ID 取得全域通知設定.
    ...
    async def create_notification_event(self, event: NotificationEvent) -> NotificationEvent:
        """建立通知事件記錄.
    ...
    async def get_notification_event_by_id(self, event_id: int) -> NotificationEvent | None:
        """根據 ID 取得通知事件.
        ...
        Returns:
            通知事件或 None
        """
        query = (QueryBuilder("notification_events")
                .select("*")
                .where("id", "=", event_id))
        ...
        if row:
            columns = [
                'id', 'user_id', 'guild_id', 'achievement_id', 'notification_type',
...
class AchievementEventRepository(BaseRepository):
    ...
    async def create_event(self, event_data: AchievementEventData) -> AchievementEventData:
        """建立新的成就事件記錄.
    ...
    async def create_events_batch(self, events: list[AchievementEventData]) -> list[AchievementEventData]:
        """批次建立多個成就事件記錄.
    ...
    async def get_events_by_user(
        ...
        try:
            ...
            query = (query.order_by("timestamp", OrderDirection.DESC)
                    .limit(limit)
                    .offset(offset))
    ...
    async def get_unprocessed_events(
        ...
        try:
            ...
            query = (query.order_by("timestamp", OrderDirection.ASC)
                    .limit(limit))
    ...
    async def get_event_stats(
        ...
        try:
            ...
            type_query = type_query.group_by("event_type").order_by("count", OrderDirection.DESC)
    ...
    async def cleanup_old_events(
        ...
        try:
            ...
            total_to_delete = self._row_to_dict(count_result).get("COUNT(*)", 0) if count_result else 0
            ...
            while deleted_count < total_to_delete:
                delete_query = (QueryBuilder(self.table_name)
                               .delete()
                               .where("timestamp", "<", cutoff_date)
                               .limit(batch_size))
    ...
    async def get_user_progress(
        ...
        if category_id:
            ...
            query = (QueryBuilder("achievement_progress", "ap")
                    .select("ap.*")
    ...
    async def create_user_progress(self, progress: AchievementProgress) -> AchievementProgress:
        """創建用戶成就進度記錄.
    ...
    async def update_user_progress(self, progress: AchievementProgress) -> AchievementProgress:
        """更新用戶成就進度記錄.
    ...
    async def delete_user_achievements_by_category(
        ...
        """
        # 需要先查詢該分類下的所有成就 ID
        achievement_query = QueryBuilder("achievements").select("id").where("category_id", category_id)
        achievement_sql, achievement_params = achievement_query.to_select_sql()
        achievement_rows = await self.execute_query(achievement_sql, achievement_params, fetch_all=True)
    ...
    async def get_achievements_count(self, category_id: int | None = None) -> int:
        ...
        Returns:
            成就總數
        """
        query = QueryBuilder("achievements").select("COUNT(*) as count").where("is_active", True)
    ...
    async def get_users_near_achievement_completion(
        ...
        try:
            # 查詢指定成就的目標值
            achievement_query = QueryBuilder("achievements")\
                .select("target_value")\
                .where("id", achievement_id)

            achievement_sql, achievement_params = achievement_query.to_select_sql()
            achievement_row = await self.execute_query(achievement_sql, achievement_params, fetch_one=True)
            ...
            # 查詢進度達到閾值的用戶
            progress_query = QueryBuilder("achievement_progress")\
            ...
            progress_rows = await self.execute_query(progress_sql, progress_params, fetch_all=True)
    ...
    async def get_achievement_progress_statistics(
        ...
        try:
            # 查詢進度統計
            stats_query = QueryBuilder("achievement_progress")\
                .select(
            ...
            stats_row = await self.execute_query(stats_sql, stats_params, fetch_one=True)
            ...
            # 計算完成率
            completion_query = QueryBuilder("achievement_progress")\
                .select("COUNT(*) as completed_users")\
                .where("achievement_id", achievement_id)\
                .where("current_value", ">=", target_value)

            completion_sql, completion_params = completion_query.to_select_sql()
            completion_row = await self.execute_query(completion_sql, completion_params, fetch_one=True)
            ...
            return {
                ...
                "target_value": target_value,
                "avg_progress_percentage": (avg_progress / target_value * 100) if target_value > 0 else 0.0
            }
    ...
    def _row_to_progress_model(self, row: Any) -> AchievementProgress:
        ...
        return AchievementProgress(
            ...
            last_updated=datetime.fromisoformat(row_dict["last_updated"]) if isinstance(row_dict["last_updated"], str) else row_dict["last_updated"]
        )
