...
class AuditLogger:
    ...
    async def _assess_risk(self, event: AuditEvent) -> None:
        ...
        try:
            # 檢查操作類型風險
            if event.event_type in [AuditEventType.BULK_RESET, AuditEventType.USER_DATA_RESET]:
                risk_level = "critical"
                requires_approval = True
    ...
    def _matches_query(self, event: AuditEvent, query: AuditQuery) -> bool:
        ...
        # 檢查用戶ID
        if query.user_ids and event.context:
            if event.context.user_id not in query.user_ids:
                return False

        # 檢查目標類型
        if query.target_types and event.target_type not in query.target_types:
            return False

        # 檢查目標ID
        if query.target_ids:
            if event.target_id not in query.target_ids and not any(
                tid in query.target_ids for tid in event.target_ids
            ):
                return False
        ...
        # 檢查成功狀態
        return not (query.success_only is not None and event.success != query.success_only)
    ...
    async def _generate_security_analysis(self, report: AuditReport) -> None:
        ...
        # 分析高風險事件
        high_risk_events = [e for e in report.events if e.risk_level in ["high", "critical"]]
        if high_risk_events:
        ...
        if len(failed_events) > 10:  # 超過10個失敗操作
            security_issues.append({
        ...
        # 檢查異常活躍用戶
        avg_activity = sum(user_activities.values()) / len(user_activities) if user_activities else 0
        ...
        report.risk_analysis = {
            ...
            "failure_rate": len(failed_events) / len(report.events) if report.events else 0,
            "unique_users": len(user_activities),
            "most_active_user": max(user_activities.items(), key=lambda x: x[1]) if user_activities else None
        }
    ...
    async def _generate_timeline_analysis(self, report: AuditReport) -> None:
        ...
        for event in report.events:
            ...
            # 按類型統計
            event_type = event.event_type.value
            hourly_data["by_type"][event_type] = hourly_data["by_type"].get(event_type, 0) + 1

            # 按嚴重性統計
            severity = event.severity.value
            hourly_data["by_severity"][severity] = hourly_data["by_severity"].get(severity, 0) + 1
