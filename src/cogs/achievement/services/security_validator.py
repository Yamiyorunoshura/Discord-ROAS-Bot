...
class SecurityValidator:
    ...
    async def check_permission(
        ...
        try:
            ...
            required_permission = risk_rule.get(
                "required_permission", PermissionLevel.BASIC
            )

            # 檢查用戶權限
            user_permission = await self._get_user_permission(user_id)
            has_permission = self._compare_permissions(
            ...
            if requires_approval:
                # 檢查是否有有效的審批
                approval = await self._check_existing_approval(
            ...
            if self.audit_logger:
                await self.audit_logger.log_event(
                    ...
                    context=AuditContext(
                        user_id=user_id, guild_id=context.get("guild_id", 0)
                    ),
    ...
    async def _get_user_permission(self, user_id: int) -> PermissionLevel:
        ...
        active_grants = [
            ...
            and (grant.expires_at is None or grant.expires_at > datetime.utcnow())
        ]
    ...
    async def validate_security_token(
        self, token_value: str, user_id: int, operation_type: str
    ) -> dict[str, Any]:
        """驗證安全令牌.
    ...
    async def _check_valid_token(
        ...
        for token in self._security_tokens.values():
            if (
                ...
                and token.operation_type == operation_type
                and not token.used
    ...
    async def create_security_challenge(
        ...
        challenge = SecurityChallenge(
            ...
            risk_level=self._risk_rules.get(operation_type, {}).get(
                "risk_level", OperationRisk.MEDIUM
            ),
        )
    ...
    async def solve_security_challenge(
        self, challenge_id: str, response: str, user_id: int
    ) -> dict[str, Any]:
        """解決安全挑戰.
    ...
    async def request_operation_approval(
        ...
        if self.audit_logger:
            await self.audit_logger.log_event(
                event_type=AuditEventType.PERMISSION_CHECK,
                context=AuditContext(
                    user_id=requested_by, guild_id=context.get("guild_id", 0)
                ),
    ...
    async def approve_operation(  # noqa: PLR0911
        ...
        if approval.status != "pending":
            return {"success": False, "reason": f"approval_already_{approval.status}"}
        ...
        if not self._compare_permissions(approver_permission, PermissionLevel.ADMIN):
            return {"success": False, "reason": "insufficient_permission_to_approve"}

        # 檢查是否是自己審批自己的請求
        if approver_id == approval.requested_by:
            return {"success": False, "reason": "cannot_approve_own_request"}
        ...
        if len(approval.current_approvers) >= approval.required_approvers:
            ...
            remaining_approvals = approval.required_approvers - len(
    ...
    async def _check_existing_approval(
        ...
        for approval in self._operation_approvals.values():
            if (
                ...
                and approval.operation_type == operation_type
    ...
    async def revoke_permission(
        self, grant_id: str, revoked_by: int, reason: str | None = None
    ) -> bool:
        """撤銷權限.
    ...
    async def _cleanup_expired_items(self) -> None:
        ...
        expired_tokens = [
            token_value
            for token_value, token in self._security_tokens.items()
        ...
        expired_challenges = [
            challenge_id
            for challenge_id, challenge in self._security_challenges.items()
