...
class RealAdminService:
    ...
    async def get_user_stats(self, user_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            total_points = sum(
                ach.points for ach in achievements if hasattr(ach, "points")
            )
    ...
    async def reset_user_category_data(self, user_id: int, category_id: int) -> bool:
        ...
        try:
            # 獲取該分類下的所有成就
            achievements = await self.repository.get_achievements_by_category(
                category_id
            )
    ...
    async def grant_achievement(
        self, user_id: int, achievement_id: int, admin_user_id: int
    ) -> bool:
        """授予用戶成就."""
        try:
            # 檢查用戶是否已有此成就
            ...
            logger.info(
                f"管理員 {admin_user_id} 為用戶 {user_id} 授予成就 {achievement_id}"
            )
            return True
    ...
    async def revoke_achievement(
        ...
        try:
            await self.repository.delete_user_achievement(user_id, achievement_id)
            logger.info(
                f"管理員 {admin_user_id} 撤銷用戶 {user_id} 的成就 {achievement_id}"
            )
            return True
        except Exception as e:
    ...
    async def update_user_progress(
        self, user_id: int, achievement_id: int, new_value: float, admin_user_id: int
    ) -> bool:
        """更新用戶成就進度."""
        try:
            # 獲取現有進度
            ...
            logger.info(
                f"管理員 {admin_user_id} 更新用戶 {user_id} 成就 {achievement_id} 進度為 {new_value}"
            )
            return True
    ...
    async def get_system_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            total_users_with_achievements = (
                await self.repository.count_users_with_achievements()
            )
    ...
    async def get_achievement_usage_stats(self, achievement_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            # 獲取獲得此成就的用戶數
            user_count = await self.repository.count_users_with_achievement(
                achievement_id
            )

            # 獲取進度中的用戶數
            progress_count = await self.repository.count_users_with_progress(
                achievement_id
            )
            ...
            return {
                ...
                "completion_rate": (user_count / (user_count + progress_count)) * 100
                if (user_count + progress_count) > 0
                else 0,
            }
        except Exception as e:
