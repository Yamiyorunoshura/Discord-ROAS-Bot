...
class PersistentConfig:
    ...
    async def update_setting(
        self, guild_id: int, category: str, key: str, value: Any
    ) -> bool:
        """更新特定設定"""
        try:
    ...
    async def get_setting(
        self, guild_id: int, category: str, key: str, default: Any = None
    ) -> Any:
        """獲取特定設定"""
        try:
            config = await self.load_config(guild_id)
    ...
    async def get_theme_config(self, guild_id: int, theme_name: str) -> dict[str, Any]:
        """獲取主題配置"""
        config = await self.load_config(guild_id)
        theme_configs = config.get(
            "theme_configs", self.default_config["theme_configs"]
        )
    ...
    async def export_config(self, guild_id: int) -> str | None:
        """匯出配置為 JSON 字串"""
        try:
            config = await self.load_config(guild_id)
