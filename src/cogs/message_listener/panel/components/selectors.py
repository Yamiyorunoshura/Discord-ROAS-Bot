    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                f"✅ 日誌頻道已設定為 {channel.mention}", ephemeral=True
            )
...
class MonitoredChannelsSelect(ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 計算添加和移除的頻道
            to_add = [
                ch_id for ch_id in selected_channels if ch_id not in current_channels
            ]
            to_remove = [
                ch_id for ch_id in current_channels if ch_id not in selected_channels
            ]
            ...
            if to_add:
                ...
                message.append(
                    f"✅ 已添加 {len(to_add)} 個監聽頻道: {', '.join(added_channels)}"
                )

            if to_remove:
                removed_channels = [f"<#{ch_id}>" for ch_id in to_remove]
                message.append(
                    f"✅ 已移除 {len(to_remove)} 個監聽頻道: {', '.join(removed_channels)}"
                )
