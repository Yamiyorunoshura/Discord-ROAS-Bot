...
class PerformanceDashboardView(StandardPanelView):
    ...
    async def toggle_auto_refresh_callback(self, interaction: discord.Interaction):
        ...
        if self.auto_refresh_task and not self.auto_refresh_task.done():
            ...
            await interaction.response.send_message(
                "▶️ 自動刷新已啟用(30秒間隔)", ephemeral=True
            )
    ...
    async def generate_report_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                "📋 性能報告已生成", file=file, ephemeral=True
            )
    ...
    async def build_overview_embed(self) -> discord.Embed:
        ...
        embed = StandardEmbedBuilder.create_info_embed(
            "📊 系統性能概覽", "Discord ADR Bot v1.6 整體性能狀態"
        )
        ...
        try:
            ...
            embed.add_field(
                name="❌ 數據載入錯誤", value=f"無法載入性能數據: {e!s}", inline=False
            )

        embed.set_footer(
            text=f"最後更新: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        return embed
        ...
        try:
            ...
            if cache_stats:
                ...
                if hot_keys:
                    ...
                    embed.add_field(
                        name="🔥 熱點鍵值", value=hot_keys_text, inline=False
                    )
            else:
                embed.add_field(
                    name="i️ 緩存狀態",
            ...
            embed.add_field(
                name="❌ 緩存數據錯誤", value=f"無法載入緩存統計: {e!s}", inline=False
            )
    ...
    async def build_events_embed(self) -> discord.Embed:
        ...
        try:
            ...
            embed.add_field(
                name="❌ 事件數據錯誤", value=f"無法載入事件統計: {e!s}", inline=False
            )
    ...
    async def build_system_embed(self) -> discord.Embed:
        ...
        try:
            ...
            embed.add_field(
                name="❌ 系統數據錯誤", value=f"無法載入系統信息: {e!s}", inline=False
            )
    ...
    async def build_alerts_embed(self) -> discord.Embed:
        ...
        try:
            ...
            embed.add_field(
                name="❌ 警報數據錯誤", value=f"無法載入警報信息: {e!s}", inline=False
            )

        embed.set_footer(
            text=f"最後檢查: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        return embed
    ...
    async def _get_performance_summary(self) -> dict[str, Any]:
        """獲取性能摘要"""
        # 這裡可以整合各個組件的性能數據
        summary = {"avg_response_time": 0, "cache_hit_rate": 0, "events_per_second": 0}
    ...
    async def _get_performance_alerts(self) -> dict[str, list[str]]:
        """獲取性能警報"""
        alerts = {"critical": [], "warnings": [], "recommendations": []}
        ...
        try:
            ...
            alerts["critical"].append(f"無法檢查系統狀態: {e!s}")
...
class PerformanceDashboard:
    ...
    async def create_dashboard(
        ...
        if interaction.user.id in self.active_dashboards:
            ...
            if (
                old_dashboard.auto_refresh_task
                and not old_dashboard.auto_refresh_task.done()
            ):
