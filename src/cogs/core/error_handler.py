...
class ErrorHandler:
    ...
    async def try_recovery(
        self, strategy_name: str, func: Callable, *args, **kwargs
    ) -> Any:
        """
        嘗試錯誤恢復
    ...
    def handle_error(
        self,
        interaction_or_ctx: commands.Context | discord.Interaction | None = None,
        user_msg: str = "發生未知錯誤",
        error_code: int = 999,
        ...
        try:
            ...
            # 生成用戶友好的錯誤訊息
            if auto_classify:
                formatted_msg = self.format_user_friendly_message(
                    exc, severity, tracking_id
                )
            else:
                formatted_msg = self.format_error_message(
                    user_msg, tracking_id, severity
                )
