...
class DependencyContainer:
    ...
    async def resolve(self, service_type: type[T], scope: str | None = None) -> T:
        ...
        if service_type in self._resolution_stack:
            cycle = (
                " -> ".join([str(t.__name__) for t in self._resolution_stack])
        ...
        if descriptor.lifetime == ServiceLifetime.SINGLETON:
            ...
            return await self._resolve_scoped(
                service_type, descriptor, scope or "default"
            )
        else:
    ...
    async def _resolve_scoped(
        self, service_type: type[T], descriptor: ServiceDescriptor, scope: str
    ) -> T:
        """解析作用域服務"""
    ...
    async def _create_instance(self, descriptor: ServiceDescriptor) -> Any:
        ...
        if descriptor.implementation_type is None:
            raise DependencyResolutionError(
                f"服務 {descriptor.service_type.__name__} 沒有實現類型"
            )
    ...
    async def dispose(self):
        ...
        try:
            ...
            for instance in self._singletons.values():
                if hasattr(instance, "dispose") and asyncio.iscoroutinefunction(
                    instance.dispose
                ):
                    try:
            ...
            "services": [
                {
                    ...
                    "implementation_type": desc.implementation_type.__name__
                    if desc.implementation_type
                    else None,
