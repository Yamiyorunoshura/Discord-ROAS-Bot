...
class BaseCog(commands.Cog):
    ...
    async def resolve_service(
        self, service_type: type[T], scope: str | None = None
    ) -> T:
        """
        解析服務實例
    ...
    def register_service(
        self,
        service_type: type[T],
        implementation_type: type[T | None] | None = None,
    ...
    def register_factory(
        self, service_type: type[T], factory, lifetime: str = "transient"
    ) -> "BaseCog":
        """
        註冊工廠方法服務
    ...
    def with_error_handler(
        self, interaction, error_message: str, error_code: int = 500
    ):
        """
        錯誤處理裝飾器
    ...
    @property
    def container(self) -> DependencyContainer | None:
        """獲取依賴容器"""
...
class BasePanelView(discord.ui.View, ABC):
    ...
    def __init__(
        ...
        *,
        timeout: float = 300.0,
        required_permissions: list[str | None] | None = None,
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        try:
            # 檢查管理員權限
            if self.admin_only and (
                not hasattr(interaction.user, "guild_permissions")
                or not interaction.user.guild_permissions.administrator
            ):
                await self._send_error_response(interaction, "只有管理員可以使用此功能")
                return False
            ...
            if self.required_permissions:
                if not hasattr(interaction.user, "guild_permissions"):
                    await self._send_error_response(interaction, "無法檢查權限")
                    return False
                ...
                if missing_permissions:
                    await self._send_error_response(
                        interaction, f"缺少必要權限: {', '.join(missing_permissions)}"
                    )
                    return False
            ...
            await self._send_error_response(interaction, "權限檢查時發生錯誤")
            return False
    ...
    async def on_error(
        ...
        # 發送錯誤響應
        await self._send_error_response(interaction, "操作時發生錯誤,請稍後再試")
...
class StandardPanelView(BasePanelView):
    ...
    async def close_callback(self, interaction: discord.Interaction):
        """關閉回調"""
        try:
            embed = StandardEmbedBuilder.create_info_embed("面板已關閉", "感謝使用!")
    ...
    async def execute_operation(
        ...
        **kwargs,
        ...
        if self.operation_in_progress:
            await self._send_error_response(
                interaction, f"{operation_name}正在進行中,請稍後再試"
            )
            return
        ...
        try:
            ...
            progress_embed = StandardEmbedBuilder.create_info_embed(
                f"{operation_name}中...", "請稍候,操作正在進行中..."
            )
            ...
            if not interaction.response.is_done():
                ...
                await interaction.edit_original_response(
                    embed=progress_embed, view=self
                )
        ...
        try:
            ...
            if self.message:
                timeout_embed = StandardEmbedBuilder.create_warning_embed(
                    "面板已超時", "面板已因超時而停用.如需繼續使用,請重新開啟面板."
                )
...
class StandardEmbedBuilder:
    ...
    def create_settings_embed(
        title: str, settings: dict[str, Any], **kwargs
    ) -> discord.Embed:
        """創建設置嵌入"""
        embed = discord.Embed(
            title=f"⚙️ {title}", color=kwargs.get("color", discord.Color.blue())
        )

        for key, value in settings.items():
            embed.add_field(
                name=key, value=str(value), inline=kwargs.get("inline", True)
            )
    ...
    def create_stats_embed(
        title: str, stats: dict[str, Any], **kwargs
    ) -> discord.Embed:
        """創建統計嵌入"""
        embed = discord.Embed(
            title=f"📊 {title}", color=kwargs.get("color", discord.Color.blue())
        )
