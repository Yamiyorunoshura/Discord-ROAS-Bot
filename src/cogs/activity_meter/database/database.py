...
from ..constants import CACHE_EXPIRY_SECONDS, MAX_HOUR, MAX_MINUTE
...
class ActivityDatabase:
    ...
    async def init_db(self) -> None:
        ...
        try:
            ...
            raise ActivityMeterError("E101", f"資料庫初始化失敗: {e}") from e
    ...
    async def get_report_channels(self) -> list[tuple[int, int]]:
        ...
        try:
            ...
            async with pool.get_connection_context(config.ACTIVITY_DB_PATH) as conn:
                cursor = await conn.execute(
                    "SELECT guild_id, channel_id FROM report_channel"
                )
                rows = await cursor.fetchall()
    ...
    async def get_monthly_top_users(
        ...
        try:
            ...
            return [
                (user_id, avg_score, message_count)
                for user_id, avg_score, message_count in results
            ]
    ...
    async def get_last_month_message_count(self) -> int:
        ...
        try:
            ...
            async with pool.get_connection_context(config.ACTIVITY_DB_PATH) as conn:
                ...
                """,
                    (
                        int(start_of_last_month.timestamp()),
                        int(start_of_month.timestamp()),
                    ),
                )
                result = await cursor.fetchone()
    ...
    async def get_latest_settings(self, guild_id: int) -> dict:
        ...
        # 檢查緩存是否過期
        if (
            cache_key not in self._cache_time
            or current_time - self._cache_time[cache_key] > CACHE_EXPIRY_SECONDS
        ):  # 30秒過期
            # 強制從數據庫重新載入
    ...
    # NumPy 優化相關的批量操作方法
    async def get_all_user_activities(self, guild_id: int) -> list[dict[str, Any]]:
        """
        獲取伺服器所有用戶的活躍度資料
    ...
    async def bulk_update_user_activities(
        ...
        Args:
            updates: 更新資料列表 [(guild_id, user_id, score, timestamp), ...]
        """
        try:
            if not updates:
                return
            ...
            logger.info(f"[活躍度]批量更新完成: {len(updates)} 筆記錄")
    ...
    async def bulk_increment_daily_messages(
        ...
        Args:
            entries: 計數項目列表 [(ymd, guild_id, user_id), ...]
        """
        try:
            if not entries:
                return
            ...
            logger.info(f"[活躍度]批量訊息計數更新完成: {len(entries)} 筆記錄")
