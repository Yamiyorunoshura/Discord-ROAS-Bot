...
from .managers import DataManager, PageManager, PermissionManager, UIManager
...
class ActivityPanelView(StandardPanelView):
    ...
    def _add_settings_components_fixed(self):
        ...
        try:
            ...
            required_rows = (
                total_components + max_components_per_row - 1
    ...
    def _check_and_optimize_layout(self):
        ...
        try:
            ...
            if not self.layout_manager.check_layout_compatibility(components):
                ...
                # 記錄優化結果
                final_layout_info = self.layout_manager.get_layout_info(
                    list(self.children)
                )
            ...
            try:
                ...
                simplified_components = self.layout_manager._create_simplified_layout(
                    components
                )
    ...
    async def auto_save_settings(
        self, interaction: discord.Interaction, setting_type: str, value: Any
    ):
        """設定變更後自動保存"""
        try:
            ...
            # 顯示保存成功提示
            await self._send_success_response(interaction, "設定已自動保存")
    ...
    async def start(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 3. 發送響應
            await interaction.response.send_message(
                embed=embed, view=self, ephemeral=False
            )
    ...
    async def handle_error(self, interaction: discord.Interaction, error: Exception):
        ...
        try:
            ...
            # 檢查是否為佈局相關錯誤
            if (
                "item would not fit at row" in error_message
                or "too many components" in error_message
            ):
                # 使用佈局錯誤處理器
    ...
    async def build_settings_embed(self) -> discord.Embed:
        ...
        embed = await create_settings_embed(
            guild, announcement_channel, progress_style, announcement_time
        )
    ...
    async def show_message_trend_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(
                name="本月訊息總量", value=f"{current_month_count:,} 則", inline=True
            )

            embed.add_field(
                name="上個月訊息總量", value=f"{last_month_count:,} 則", inline=True
            )

            embed.add_field(
                name="變化趨勢", value=f"{change_emoji} {change_text}", inline=False
            )
    ...
    async def render_progress_preview(self, style: str) -> discord.File:
        ...
        try:
            ...
            # 進度條
            draw.rectangle(
                [10, 15, 10 + progress_width, height - 15], fill=progress_color
            )
    ...
    async def update_announcement_channel(
        ...
        try:
            ...
            # 使用自動保存機制
            await self.auto_save_settings(
                interaction, "announcement_channel", channel_id
            )

        except Exception as e:
            await self.handle_error(interaction, e)

    async def update_announcement_time(
        self, interaction: discord.Interaction, hour: int
    ):
        """更新公告時間(已棄用,使用自動保存機制)"""
        try:
            # 檢查時間是否有效
