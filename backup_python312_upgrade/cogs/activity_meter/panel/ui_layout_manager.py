...
class DiscordUILayoutManager:
    ...
    def check_layout_compatibility(self, components: List[discord.ui.Item]) -> bool:
        ...
        if len(components) > DiscordUILayoutManager.max_total_components:
            logger.warning(f"組件總數超過限制: {len(components)} > {DiscordUILayoutManager.max_total_components}")
            return False
        ...
        for component in components:
            ...
            if row_counts[row] > DiscordUILayoutManager.max_components_per_row:
                logger.warning(f"第 {row} 行組件數量超過限制: {row_counts[row]} > {DiscordUILayoutManager.max_components_per_row}")
                return False
        
        # 檢查行數是否超過限制
        if row_counts and max(row_counts.keys()) >= DiscordUILayoutManager.max_rows:
            logger.warning(f"行數超過限制: {max(row_counts.keys())} >= {DiscordUILayoutManager.max_rows}")
            return False
        
        return True
    
    def optimize_layout(self, components: List[discord.ui.Item]) -> List[discord.ui.Item]:
        """
        優化佈局 - 改進版本
    ...
    def _create_simplified_layout(self, components: List[discord.ui.Item]) -> List[discord.ui.Item]:
        """
        創建簡化佈局 - 當優化失敗時的備用方案
...
class UILayoutErrorHandler:
    ...
    async def handle_layout_error(self, interaction: discord.Interaction, error: Exception):
        """
        處理佈局錯誤 - 改進版本
    ...
    async def handle_general_error(self, interaction: discord.Interaction, error: Exception):
        """
        處理一般錯誤
        ...
        embed = discord.Embed(
            title="❌ 發生錯誤",
            description=f"���生未預期的錯誤：{str(error)}",
            color=discord.Color.red()
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
