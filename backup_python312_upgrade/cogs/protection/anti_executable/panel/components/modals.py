...
class SettingsModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            notify_channel = self.children[3].value.strip() if self.children[3].value else None
...
class AddWhitelistModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 新增到白名單
            await self.main_view.cog.add_whitelist(
                self.main_view.guild_id,
                item,
                note
            )
...
class AddBlacklistModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 新增到黑名單
            await self.main_view.cog.add_blacklist(
                self.main_view.guild_id,
                item,
                note
            )
...
class AddFormatModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            description = self.children[1].value.strip() if self.children[1].value else None
...
class RemoveFormatModal(BaseModal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if success:
                await interaction.response.send_message(
                    f"✅ 已移除檔案格式：{format_name}",
                    ephemeral=True
                )
            else:
                await interaction.response.send_message(
                    f"❌ 找不到檔案格式：{format_name}",
                    ephemeral=True
                )
