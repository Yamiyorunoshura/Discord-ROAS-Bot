...
import discord
from discord.ui import View, Button, ChannelSelect, Select, Modal, TextInput
from typing import List, Dict, Optional, Any, Union

from ..config.config import is_allowed
from .embeds import settings_embed, preview_embed, stats_embed
from .components.buttons import (
    HelpButton, AdjustBatchSize, AdjustBatchTime, 
    LogChannelSelect, MonitoredChannelsSelect, 
    ToggleEdits, ToggleDeletes, CloseButton,
    PageButton,
    # 新增的按鈕組件
    SmartBatchConfigButton, RenderQualityButton, FontSettingsButton,
    ColorThemeButton, RenderPreviewButton
)
        ...
        try:
            ...
            if processor and hasattr(processor, 'batch_processor'):
                ...
                if bp.performance_history:
                    ...
                    avg_time = sum(p['processing_time'] for p in recent_perf) / len(recent_perf)
                    avg_success = sum(p['success_rate'] for p in recent_perf) / len(recent_perf)
...
class SearchPaginationView(View):
    ...
    def build_page_embed(self) -> discord.Embed:
        ...
        if not current_results:
            embed.add_field(name="無結果", value="沒有找到符合條件的訊息。", inline=False)
            return embed
    ...
    def _update_buttons(self):
        ...
        self.add_item(PageButton(self, 1, "下一頁 ➡️", disabled=(self.page >= self.total_pages - 1)))
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message("❌ 只有原始搜尋者可以使用這些按鈕。", ephemeral=True)
            return False
        return True
