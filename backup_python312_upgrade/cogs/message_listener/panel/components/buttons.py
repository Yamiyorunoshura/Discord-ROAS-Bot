...
import discord
from discord.ui import Button, ChannelSelect, Select, Modal, TextInput
...
class SmartBatchConfigButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not processor:
                await interaction.response.send_message("❌ 處理器未初始化", ephemeral=True)
                return
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if quality not in ['low', 'medium', 'high']:
                await interaction.response.send_message("❌ 品質設定無效，請輸入 low、medium 或 high", ephemeral=True)
                return
            
            format_type = self.image_format.value.upper()
            if format_type not in ['PNG', 'JPEG']:
                await interaction.response.send_message("❌ 格式設定無效，請輸入 PNG 或 JPEG", ephemeral=True)
                return
            
            try:
                size = int(self.image_size.value)
                if size < 800 or size > 1200:
                    await interaction.response.send_message("❌ 尺寸設定無效，請輸入 800-1200 之間的數值", ephemeral=True)
                    return
            except ValueError:
                await interaction.response.send_message("❌ 尺寸必須是數字", ephemeral=True)
                return
        ...
        try:
            ...
            try:
                ...
                if size < 12 or size > 24:
                    await interaction.response.send_message("❌ 字體大小無效，請輸入 12-24 之間的數值", ephemeral=True)
                    return
            except ValueError:
            ...
            try:
                ...
                if line_height < 1.2 or line_height > 2.0:
                    await interaction.response.send_message("❌ 行高無效，請輸入 1.2-2.0 之間的數值", ephemeral=True)
                    return
            except ValueError:
                await interaction.response.send_message("❌ 行高必須是數字", ephemeral=True)
                return
...
class ColorThemeSelect(Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            theme_config = theme_configs.get(selected_theme, theme_configs["discord_default"])
            ...
            await interaction.response.send_message(f"❌ 主題設定失敗：{str(e)}", ephemeral=True)
...
class LogChannelSelect(ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"已選擇日誌頻道：{selected_channel.mention}", ephemeral=True)
