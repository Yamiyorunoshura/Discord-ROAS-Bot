...
class AvatarDownloader:
    ...
    async def download_with_retry(self, url: str) -> Optional[bytes]:
        """
        帶重試機制的下載
        ...
        Returns:
            Optional[bytes]: 下載的數據，失敗時返回 None
        """
...
class WelcomeRenderer:
    ...
    async def generate_welcome_image(
        ...
        Returns:
            Optional[io.BytesIO]: 圖片的二進位資料流，如果失敗則為 None
        """
        try:
            # 獲取模板
    ...
    def _create_canvas(self, layout: LayoutConfig, template: WelcomeTemplate, 
                      bg_path: Optional[str]) -> Image.Image:
        """
        創建畫布
