...
class TestModalStaticMethods:
    ...
    def test_validate_time_format_valid(self):
        """測試有效時間格式驗證"""
        # 直接測試靜態方法，不創建Modal實例
        from cogs.activity_meter.panel.components.modals import AnnouncementTimeModal
        ...
        for time_str in valid_times:
            ...
            assert result == True, f"時間格式 {time_str} 應該有效"
    ...
    def test_validate_time_format_invalid(self):
        ...
        for time_str in invalid_times:
            result = validator.validate_time_format(time_str)
            assert result == False, f"時間格式 {time_str} 應該無效"
...
class TestModalIntegration:
    ...
    def test_modal_time_validation_logic(self):
        ...
        valid_times = ["09:00", "12:30", "23:59", "00:00"]
        for time_str in valid_times:
            assert validate_time_format(time_str) == True, f"時間格式 {time_str} 應該有效"
        
        # 測試無效時間
        invalid_times = ["25:00", "12:60", "09:5", "abc"]
        for time_str in invalid_times:
            assert validate_time_format(time_str) == False, f"時間格式 {time_str} 應該無效"
    ...
    def test_modal_input_configuration_validation(self):
        ...
        assert expected_time_config['required'] == True
        assert expected_description_config['required'] == False
